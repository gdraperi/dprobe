...
func (s *DockerSuite) TestClientSetsTLSServerName(c *check.C) ***REMOVED***
	c.Skip("Flakey test")
	// there may be more than one hit to the server for each registry request
	serverNameReceived := []string***REMOVED******REMOVED***
	var serverName string

	virtualHostServer := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) ***REMOVED***
	...
	***REMOVED***))
	...
	for _, item := range serverNameReceived ***REMOVED***
