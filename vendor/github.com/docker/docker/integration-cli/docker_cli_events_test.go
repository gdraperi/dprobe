...
func (s *DockerSuite) TestEventsTimestampFormats(c *check.C) ***REMOVED***
	...
	unixTs := func(t time.Time) string ***REMOVED*** return fmt.Sprintf("%v", t.Unix()) ***REMOVED***
	rfc3339 := func(t time.Time) string ***REMOVED*** return t.Format(time.RFC3339) ***REMOVED***
	duration := func(t time.Time) string ***REMOVED*** return time.Since(t).String() ***REMOVED***

	// --since=$start must contain only the 'untag' event
	for _, f := range []func(time.Time) string***REMOVED***unixTs, rfc3339, duration***REMOVED*** ***REMOVED***
...
func (s *DockerSuite) TestEventsUntag(c *check.C) ***REMOVED***
	image := "busybox"
	...
	result := icmd.RunCmd(icmd.Cmd***REMOVED***
		Command: []string***REMOVED***dockerBinary, "events", "--since=1"***REMOVED***,
		Timeout: time.Millisecond * 2500,
	***REMOVED***)
	result.Assert(c, icmd.Expected***REMOVED***Timeout: true***REMOVED***)
	...
	// looking for.
	for _, v := range events[nEvents-3 : nEvents-1] ***REMOVED***
		c.Assert(v, checker.Contains, "untag", check.Commentf("event should be untag"))
	***REMOVED***
***REMOVED***

func (s *DockerSuite) TestEventsContainerEvents(c *check.C) ***REMOVED***
...
func (s *DockerSuite) TestEventsContainerEventsAttrSort(c *check.C) ***REMOVED***
	...
	for _, event := range events ***REMOVED***
		...
		if matches["eventType"] == "container" && matches["action"] == "create" ***REMOVED***
			matchedEvents++
			c.Assert(out, checker.Contains, "(image=busybox, name=container-events-test)", check.Commentf("Event attributes not sorted"))
		***REMOVED*** else if matches["eventType"] == "container" && matches["action"] == "start" ***REMOVED***
			matchedEvents++
			c.Assert(out, checker.Contains, "(image=busybox, name=container-events-test)", check.Commentf("Event attributes not sorted"))
		***REMOVED***
	***REMOVED***
...
func (s *DockerSuite) TestEventsContainerEventsSinceUnixEpoch(c *check.C) ***REMOVED***
...
func (s *DockerSuite) TestEventsImageTag(c *check.C) ***REMOVED***
...
func (s *DockerSuite) TestEventsImagePull(c *check.C) ***REMOVED***
...
func (s *DockerSuite) TestEventsImageImport(c *check.C) ***REMOVED***
...
func (s *DockerSuite) TestEventsImageLoad(c *check.C) ***REMOVED***
	testRequires(c, DaemonIsLinux)
...
func (s *DockerSuite) TestEventsPluginOps(c *check.C) ***REMOVED***
...
func (s *DockerSuite) TestEventsFilters(c *check.C) ***REMOVED***
	since := daemonUnixTime(c)
...
func (s *DockerSuite) TestEventsFilterImageName(c *check.C) ***REMOVED***
	since := daemonUnixTime(c)
	...
	for _, e := range events ***REMOVED***
		if strings.Contains(e, container1) ***REMOVED***
			count1++
		***REMOVED*** else if strings.Contains(e, container2) ***REMOVED***
			count2++
		***REMOVED***
	***REMOVED***
...
func (s *DockerSuite) TestEventsFilterLabels(c *check.C) ***REMOVED***
	since := daemonUnixTime(c)
...
func (s *DockerSuite) TestEventsFilterImageLabels(c *check.C) ***REMOVED***
	since := daemonUnixTime(c)
...
func (s *DockerSuite) TestEventsFilterContainer(c *check.C) ***REMOVED***
	since := daemonUnixTime(c)
	nameID := make(map[string]string)

	for _, name := range []string***REMOVED***"container_1", "container_2"***REMOVED*** ***REMOVED***
	...
	***REMOVED***
	...
	checkEvents := func(id string, events []string) error ***REMOVED***
		if len(events) != 4 ***REMOVED*** // create, attach, start, die
			return fmt.Errorf("expected 4 events, got %v", events)
		***REMOVED***
		for _, event := range events ***REMOVED***
			matches := eventstestutils.ScanMap(event)
			if !matchEventID(matches, id) ***REMOVED***
	...
	***REMOVED***
	...
	for name, ID := range nameID ***REMOVED***
		// filter by names
...
func (s *DockerSuite) TestEventsCommit(c *check.C) ***REMOVED***
...
func (s *DockerSuite) TestEventsCopy(c *check.C) ***REMOVED***
	// Build a test image.
...
func (s *DockerSuite) TestEventsResize(c *check.C) ***REMOVED***
...
func (s *DockerSuite) TestEventsAttach(c *check.C) ***REMOVED***
...
func (s *DockerSuite) TestEventsRename(c *check.C) ***REMOVED***
...
func (s *DockerSuite) TestEventsTop(c *check.C) ***REMOVED***
...
// #14316
func (s *DockerRegistrySuite) TestEventsImageFilterPush(c *check.C) ***REMOVED***
...
func (s *DockerSuite) TestEventsFilterType(c *check.C) ***REMOVED***
	since := daemonUnixTime(c)
	name := "labelfiltertest"
	label := "io.docker.testing=image"
...
// #25798
func (s *DockerSuite) TestEventsSpecialFiltersWithExecCreate(c *check.C) ***REMOVED***
	since := daemonUnixTime(c)
...
func (s *DockerSuite) TestEventsFilterImageInContainerAction(c *check.C) ***REMOVED***
	since := daemonUnixTime(c)
...
func (s *DockerSuite) TestEventsContainerRestart(c *check.C) ***REMOVED***
	...
	***REMOVED***
	...
	err := waitInspect("testEvent", "***REMOVED******REMOVED*** .State.Restarting ***REMOVED******REMOVED*** ***REMOVED******REMOVED*** .State.Running ***REMOVED******REMOVED***", "false false", waitTime)
	c.Assert(err, checker.IsNil)
...
func (s *DockerSuite) TestEventsSinceInTheFuture(c *check.C) ***REMOVED***
	dockerCmd(c, "run", "--name", "test-container", "-d", "busybox", "true")
	waitRun("test-container")
...
func (s *DockerSuite) TestEventsUntilInThePast(c *check.C) ***REMOVED***
	since := daemonUnixTime(c)
...
func (s *DockerSuite) TestEventsFormat(c *check.C) ***REMOVED***
	since := daemonUnixTime(c)
	dockerCmd(c, "run", "--rm", "busybox", "true")
	dockerCmd(c, "run", "--rm", "busybox", "true")
	out, _ := dockerCmd(c, "events", "--since", since, "--until", daemonUnixTime(c), "--format", "***REMOVED******REMOVED***json .***REMOVED******REMOVED***")
	...
	for ***REMOVED***
		...
		if err = dec.Decode(&ev); err == io.EOF ***REMOVED***
			break
		***REMOVED***
		c.Assert(err, checker.IsNil)
		if ev.Status == "start" ***REMOVED***
			startCount++
		***REMOVED***
	***REMOVED***
...
func (s *DockerSuite) TestEventsFormatBadFunc(c *check.C) ***REMOVED***
	// make sure it fails immediately, without receiving any event
	result := dockerCmdWithResult("events", "--format", "***REMOVED******REMOVED***badFuncString .***REMOVED******REMOVED***")
...
func (s *DockerSuite) TestEventsFormatBadField(c *check.C) ***REMOVED***
	// make sure it fails immediately, without receiving any event
	result := dockerCmdWithResult("events", "--format", "***REMOVED******REMOVED***.badFieldString***REMOVED******REMOVED***")
	result.Assert(c, icmd.Expected***REMOVED***
		Error:    "exit status 64",
		ExitCode: 64,
