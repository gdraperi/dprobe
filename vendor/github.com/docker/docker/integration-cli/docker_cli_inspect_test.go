...
func checkValidGraphDriver(c *check.C, name string) ***REMOVED***
	if name != "devicemapper" && name != "overlay" && name != "vfs" && name != "zfs" && name != "btrfs" && name != "aufs" ***REMOVED***
		c.Fatalf("%v is not a valid graph driver name", name)
	***REMOVED***
***REMOVED***

func (s *DockerSuite) TestInspectImage(c *check.C) ***REMOVED***
...
func (s *DockerSuite) TestInspectInt64(c *check.C) ***REMOVED***
...
func (s *DockerSuite) TestInspectDefault(c *check.C) ***REMOVED***
...
func (s *DockerSuite) TestInspectStatus(c *check.C) ***REMOVED***
...
func (s *DockerSuite) TestInspectTypeFlagContainer(c *check.C) ***REMOVED***
	//Both the container and image are named busybox. docker inspect will fetch container
	//JSON State.Running field. If the field is true, it's a container.
	runSleepingContainer(c, "--name=busybox", "-d")

	formatStr := "--format=***REMOVED******REMOVED***.State.Running***REMOVED******REMOVED***"
	out, _ := dockerCmd(c, "inspect", "--type=container", formatStr, "busybox")
	c.Assert(out, checker.Equals, "true\n") // not a container JSON
***REMOVED***

func (s *DockerSuite) TestInspectTypeFlagWithNoContainer(c *check.C) ***REMOVED***
...
func (s *DockerSuite) TestInspectTypeFlagWithImage(c *check.C) ***REMOVED***
...
func (s *DockerSuite) TestInspectTypeFlagWithInvalidValue(c *check.C) ***REMOVED***
...
func (s *DockerSuite) TestInspectImageFilterInt(c *check.C) ***REMOVED***
	testRequires(c, DaemonIsLinux)
	imageTest := "emptyfs"
	...
	formatStr := fmt.Sprintf("--format=***REMOVED******REMOVED***eq .Size %d***REMOVED******REMOVED***", size)
...
func (s *DockerSuite) TestInspectContainerFilterInt(c *check.C) ***REMOVED***
	...
	***REMOVED***)
	...
	formatStr := fmt.Sprintf("--format=***REMOVED******REMOVED***eq .State.ExitCode %d***REMOVED******REMOVED***", exitCode)
...
func (s *DockerSuite) TestInspectImageGraphDriver(c *check.C) ***REMOVED***
...
func (s *DockerSuite) TestInspectContainerGraphDriver(c *check.C) ***REMOVED***
	testRequires(c, DaemonIsLinux, Devicemapper)
...
func (s *DockerSuite) TestInspectBindMountPoint(c *check.C) ***REMOVED***
	modifier := ",z"
	...
	***REMOVED***
	...
	if testEnv.OSType != "windows" ***REMOVED*** // Windows does not set mode
		c.Assert(m.Mode, checker.Equals, "ro"+modifier)
	***REMOVED***
	c.Assert(m.RW, checker.Equals, false)
***REMOVED***

func (s *DockerSuite) TestInspectNamedMountPoint(c *check.C) ***REMOVED***
	prefix, slash := getPrefixAndSlashFromDaemonPlatform()
...
// #14947
func (s *DockerSuite) TestInspectTimesAsRFC3339Nano(c *check.C) ***REMOVED***
	out, _ := dockerCmd(c, "run", "-d", "busybox", "true")
	id := strings.TrimSpace(out)
...
// #15633
func (s *DockerSuite) TestInspectLogConfigNoType(c *check.C) ***REMOVED***
...
func (s *DockerSuite) TestInspectNoSizeFlagContainer(c *check.C) ***REMOVED***
	...
	formatStr := "--format=***REMOVED******REMOVED***.SizeRw***REMOVED******REMOVED***,***REMOVED******REMOVED***.SizeRootFs***REMOVED******REMOVED***"
	out, _ := dockerCmd(c, "inspect", "--type=container", formatStr, "busybox")
	c.Assert(strings.TrimSpace(out), check.Equals, "<nil>,<nil>", check.Commentf("Expected not to display size info: %s", out))
***REMOVED***

func (s *DockerSuite) TestInspectSizeFlagContainer(c *check.C) ***REMOVED***
	runSleepingContainer(c, "--name=busybox", "-d")
...
func (s *DockerSuite) TestInspectTemplateError(c *check.C) ***REMOVED***
	// Template parsing error for both the container and image.

	runSleepingContainer(c, "--name=container1", "-d")

	out, _, err := dockerCmdWithError("inspect", "--type=container", "--format='Format container: ***REMOVED******REMOVED***.ThisDoesNotExist***REMOVED******REMOVED***'", "container1")
	c.Assert(err, check.Not(check.IsNil))
	c.Assert(out, checker.Contains, "Template parsing error")

	out, _, err = dockerCmdWithError("inspect", "--type=image", "--format='Format container: ***REMOVED******REMOVED***.ThisDoesNotExist***REMOVED******REMOVED***'", "busybox")
	c.Assert(err, check.Not(check.IsNil))
	c.Assert(out, checker.Contains, "Template parsing error")
***REMOVED***

func (s *DockerSuite) TestInspectJSONFields(c *check.C) ***REMOVED***
	runSleepingContainer(c, "--name=busybox", "-d")
	out, _, err := dockerCmdWithError("inspect", "--type=container", "--format=***REMOVED******REMOVED***.HostConfig.Dns***REMOVED******REMOVED***", "busybox")

	c.Assert(err, check.IsNil)
	c.Assert(out, checker.Equals, "[]\n")
***REMOVED***

func (s *DockerSuite) TestInspectByPrefix(c *check.C) ***REMOVED***
...
func (s *DockerSuite) TestInspectStopWhenNotFound(c *check.C) ***REMOVED***
	runSleepingContainer(c, "--name=busybox1", "-d")
	runSleepingContainer(c, "--name=busybox2", "-d")
	result := dockerCmdWithResult("inspect", "--type=container", "--format='***REMOVED******REMOVED***.Name***REMOVED******REMOVED***'", "busybox1", "busybox2", "missing")
	...
	result = dockerCmdWithResult("inspect", "--type=container", "--format='***REMOVED******REMOVED***.Name***REMOVED******REMOVED***'", "missing", "busybox1", "busybox2")
...
func (s *DockerSuite) TestInspectHistory(c *check.C) ***REMOVED***
	dockerCmd(c, "run", "--name=testcont", "busybox", "echo", "hello")
	dockerCmd(c, "commit", "-m", "test comment", "testcont", "testimg")
	out, _, err := dockerCmdWithError("inspect", "--format='***REMOVED******REMOVED***.Comment***REMOVED******REMOVED***'", "testimg")
	c.Assert(err, check.IsNil)
	c.Assert(out, checker.Contains, "test comment")
***REMOVED***

func (s *DockerSuite) TestInspectContainerNetworkDefault(c *check.C) ***REMOVED***
	testRequires(c, DaemonIsLinux)

	contName := "test1"
	dockerCmd(c, "run", "--name", contName, "-d", "busybox", "top")
	netOut, _ := dockerCmd(c, "network", "inspect", "--format=***REMOVED******REMOVED***.ID***REMOVED******REMOVED***", "bridge")
...
func (s *DockerSuite) TestInspectContainerNetworkCustom(c *check.C) ***REMOVED***
	testRequires(c, DaemonIsLinux)
...
func (s *DockerSuite) TestInspectRootFS(c *check.C) ***REMOVED***
...
func (s *DockerSuite) TestInspectAmpersand(c *check.C) ***REMOVED***
	testRequires(c, DaemonIsLinux)
...
func (s *DockerSuite) TestInspectPlugin(c *check.C) ***REMOVED***
	testRequires(c, DaemonIsLinux, IsAmd64, Network)
	_, _, err := dockerCmdWithError("plugin", "install", "--grant-all-permissions", pNameWithTag)
	c.Assert(err, checker.IsNil)

	out, _, err := dockerCmdWithError("inspect", "--type", "plugin", "--format", "***REMOVED******REMOVED***.Name***REMOVED******REMOVED***", pNameWithTag)
	c.Assert(err, checker.IsNil)
	c.Assert(strings.TrimSpace(out), checker.Equals, pNameWithTag)

	out, _, err = dockerCmdWithError("inspect", "--format", "***REMOVED******REMOVED***.Name***REMOVED******REMOVED***", pNameWithTag)
	c.Assert(err, checker.IsNil)
	c.Assert(strings.TrimSpace(out), checker.Equals, pNameWithTag)
...
// Test case for 29185
func (s *DockerSuite) TestInspectUnknownObject(c *check.C) ***REMOVED***
...
func (s *DockerSuite) TestInspectInvalidReference(c *check.C) ***REMOVED***
	// This test should work on both Windows and Linux
	out, _, err := dockerCmdWithError("inspect", "FooBar")
	c.Assert(err, checker.NotNil)
