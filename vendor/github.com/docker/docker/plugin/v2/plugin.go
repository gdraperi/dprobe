...
type ErrInadequateCapability struct ***REMOVED***
	cap string
***REMOVED***

func (e ErrInadequateCapability) Error() string ***REMOVED***
...
func (p *Plugin) BasePath() string ***REMOVED***
	return p.Rootfs
***REMOVED***

// Client returns the plugin client.
func (p *Plugin) Client() *plugins.Client ***REMOVED***
	p.mu.RLock()
	defer p.mu.RUnlock()
...
func (p *Plugin) SetPClient(client *plugins.Client) ***REMOVED***
	p.mu.Lock()
	defer p.mu.Unlock()
...
func (p *Plugin) FilterByCap(capability string) (*Plugin, error) ***REMOVED***
	capability = strings.ToLower(capability)
	for _, typ := range p.PluginObj.Config.Interface.Types ***REMOVED***
		if typ.Capability == capability && typ.Prefix == "docker" ***REMOVED***
			return p, nil
		***REMOVED***
	***REMOVED***
	return nil, ErrInadequateCapability***REMOVED***capability***REMOVED***
***REMOVED***

// InitEmptySettings initializes empty settings for a plugin.
func (p *Plugin) InitEmptySettings() ***REMOVED***
	...
	for _, env := range p.PluginObj.Config.Env ***REMOVED***
		if env.Value != nil ***REMOVED***
...
func (p *Plugin) Set(args []string) error ***REMOVED***
	p.mu.Lock()
	defer p.mu.Unlock()

	if p.PluginObj.Enabled ***REMOVED***
...
next:
	for _, s := range sets ***REMOVED***
		// range over all the envs in the config
		for _, env := range p.PluginObj.Config.Env ***REMOVED***
			// found the env in the config
			if env.Name == s.name ***REMOVED***
				// is it settable ?
				if ok, err := s.isSettable(allowedSettableFieldsEnv, env.Settable); err != nil ***REMOVED***
					return err
				***REMOVED*** else if !ok ***REMOVED***
		...
		***REMOVED***
		...
		for _, mount := range p.PluginObj.Config.Mounts ***REMOVED***
			// found the mount in the config
			if mount.Name == s.name ***REMOVED***
				// is it settable ?
				if ok, err := s.isSettable(allowedSettableFieldsMounts, mount.Settable); err != nil ***REMOVED***
					return err
				***REMOVED*** else if !ok ***REMOVED***
					return fmt.Errorf("%q is not settable", s.prettyName())
				***REMOVED***
		...
		***REMOVED***
		...
		for _, device := range p.PluginObj.Config.Linux.Devices ***REMOVED***
			// found the device in the config
			if device.Name == s.name ***REMOVED***
				// is it settable ?
				if ok, err := s.isSettable(allowedSettableFieldsDevices, device.Settable); err != nil ***REMOVED***
					return err
				***REMOVED*** else if !ok ***REMOVED***
					return fmt.Errorf("%q is not settable", s.prettyName())
				***REMOVED***
		...
		***REMOVED***
		...
		if p.PluginObj.Config.Args.Name == s.name ***REMOVED***
			// is it settable ?
			if ok, err := s.isSettable(allowedSettableFieldsArgs, p.PluginObj.Config.Args.Settable); err != nil ***REMOVED***
				return err
			***REMOVED*** else if !ok ***REMOVED***
				return fmt.Errorf("%q is not settable", s.prettyName())
			***REMOVED***
...
func (p *Plugin) IsEnabled() bool ***REMOVED***
	p.mu.RLock()
	defer p.mu.RUnlock()
...
func (p *Plugin) GetID() string ***REMOVED***
	p.mu.RLock()
	defer p.mu.RUnlock()
...
func (p *Plugin) GetSocket() string ***REMOVED***
	p.mu.RLock()
	defer p.mu.RUnlock()
...
func (p *Plugin) GetTypes() []types.PluginInterfaceType ***REMOVED***
	p.mu.RLock()
	defer p.mu.RUnlock()
...
func (p *Plugin) GetRefCount() int ***REMOVED***
	p.mu.RLock()
	defer p.mu.RUnlock()
...
func (p *Plugin) AddRefCount(count int) ***REMOVED***
	p.mu.Lock()
	defer p.mu.Unlock()
