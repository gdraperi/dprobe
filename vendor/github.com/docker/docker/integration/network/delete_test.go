...
func containsNetwork(nws []types.NetworkResource, nw types.NetworkCreateResponse) bool ***REMOVED***
	for _, n := range nws ***REMOVED***
		if n.ID == nw.ID ***REMOVED***
			return true
		***REMOVED***
	***REMOVED***
	return false
...
func createAmbiguousNetworks(t *testing.T) (types.NetworkCreateResponse, types.NetworkCreateResponse, types.NetworkCreateResponse) ***REMOVED***
	client := request.NewAPIClient(t)
	ctx := context.Background()

	testNet, err := client.NetworkCreate(ctx, "testNet", types.NetworkCreate***REMOVED******REMOVED***)
	require.NoError(t, err)
	idPrefixNet, err := client.NetworkCreate(ctx, testNet.ID[:12], types.NetworkCreate***REMOVED******REMOVED***)
	require.NoError(t, err)
	fullIDNet, err := client.NetworkCreate(ctx, testNet.ID, types.NetworkCreate***REMOVED******REMOVED***)
	require.NoError(t, err)

	nws, err := client.NetworkList(ctx, types.NetworkListOptions***REMOVED******REMOVED***)
	require.NoError(t, err)
...
func TestDockerNetworkDeletePreferID(t *testing.T) ***REMOVED***
	defer setupTest(t)()
	client := request.NewAPIClient(t)
	ctx := context.Background()
