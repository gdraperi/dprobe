...
func decode(buf []byte, out interface***REMOVED******REMOVED***) error ***REMOVED***
...
func encode(msgType messageType, in interface***REMOVED******REMOVED***) (*bytes.Buffer, error) ***REMOVED***
...
// a node is suspected
func suspicionTimeout(suspicionMult, n int, interval time.Duration) time.Duration ***REMOVED***
...
func retransmitLimit(retransmitMult, n int) int ***REMOVED***
...
func shuffleNodes(nodes []*nodeState) ***REMOVED***
	n := len(nodes)
...
// cluster size grows
func pushPullScale(interval time.Duration, n int) time.Duration ***REMOVED***
	// Don't scale until we cross the threshold
	if n <= pushPullScaleThreshold ***REMOVED***
		return interval
	***REMOVED***
...
func moveDeadNodes(nodes []*nodeState, gossipToTheDeadTime time.Duration) int ***REMOVED***
	numDead := 0
	n := len(nodes)
	for i := 0; i < n-numDead; i++ ***REMOVED***
		if nodes[i].State != stateDead ***REMOVED***
			continue
		***REMOVED***

		// Respect the gossip to the dead interval
		if time.Since(nodes[i].StateChange) <= gossipToTheDeadTime ***REMOVED***
			continue
		***REMOVED***
...
// returned.
func kRandomNodes(k int, nodes []*nodeState, filterFn func(*nodeState) bool) []*nodeState ***REMOVED***
	n := len(nodes)
...
OUTER:
	...
	// exhaustive
	for i := 0; i < 3*n && len(kNodes) < k; i++ ***REMOVED***
		// Get random node
		idx := randomOffset(n)
		node := nodes[idx]

		// Give the filter a shot at it.
		if filterFn != nil && filterFn(node) ***REMOVED***
			continue OUTER
		***REMOVED***

		// Check if we have this node already
		for j := 0; j < len(kNodes); j++ ***REMOVED***
			if node == kNodes[j] ***REMOVED***
				continue OUTER
			***REMOVED***
		***REMOVED***
...
func makeCompoundMessage(msgs [][]byte) *bytes.Buffer ***REMOVED***
...
func decodeCompoundMessage(buf []byte) (trunc int, parts [][]byte, err error) ***REMOVED***
	if len(buf) < 1 ***REMOVED***
	...
	***REMOVED***
	...
	if len(buf) < int(numParts*2) ***REMOVED***
	...
	***REMOVED***
	...
	for i := 0; i < int(numParts); i++ ***REMOVED***
		lengths[i] = binary.BigEndian.Uint16(buf[i*2 : i*2+2])
	***REMOVED***
	buf = buf[numParts*2:]

	// Split each message
	for idx, msgLen := range lengths ***REMOVED***
		if len(buf) < int(msgLen) ***REMOVED***
...
func hasPort(s string) bool ***REMOVED***
...
// and wraps it in a compress***REMOVED******REMOVED*** message that is encoded.
func compressPayload(inp []byte) (*bytes.Buffer, error) ***REMOVED***
	...
	***REMOVED***
	...
	if err := compressor.Close(); err != nil ***REMOVED***
		return nil, err
	***REMOVED***
...
// decompressPayload is used to unpack an encoded compress***REMOVED******REMOVED***
// message and return its payload uncompressed
func decompressPayload(msg []byte) ([]byte, error) ***REMOVED***
	// Decode the message
	var c compress
	if err := decode(msg, &c); err != nil ***REMOVED***
		return nil, err
	***REMOVED***
...
func decompressBuffer(c *compress) ([]byte, error) ***REMOVED***
	// Verify the algorithm
	if c.Algo != lzwAlgo ***REMOVED***
...
// transport.
func joinHostPort(host string, port uint16) string ***REMOVED***
