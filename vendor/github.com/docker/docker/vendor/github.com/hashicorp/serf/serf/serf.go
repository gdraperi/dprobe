...
func (s SerfState) String() string ***REMOVED***
	switch s ***REMOVED***
	case SerfAlive:
		return "alive"
	case SerfLeaving:
...
func (s MemberStatus) String() string ***REMOVED***
	switch s ***REMOVED***
	case StatusNone:
		return "none"
	case StatusAlive:
...
type memberState struct ***REMOVED***
	Member
...
type nodeIntent struct ***REMOVED***
	LTime LamportTime
	Node  string
***REMOVED***
...
func (ue *userEvent) Equals(other *userEvent) bool ***REMOVED***
	if ue.Name != other.Name ***REMOVED***
		return false
	***REMOVED***
	if bytes.Compare(ue.Payload, other.Payload) != 0 ***REMOVED***
		return false
	***REMOVED***
	return true
***REMOVED***
...
type userEvents struct ***REMOVED***
	LTime  LamportTime
	Events []userEvent
***REMOVED***

// queries stores all the query ids at a specific time
type queries struct ***REMOVED***
	LTime    LamportTime
	QueryIDs []uint32
***REMOVED***
...
func Create(conf *Config) (*Serf, error) ***REMOVED***
	conf.Init()
	...
	***REMOVED***
	...
	if len(serf.encodeTags(conf.Tags)) > memberlist.MetaMaxSize ***REMOVED***
	...
	***REMOVED***
	...
	if conf.CoalescePeriod > 0 && conf.QuiescentPeriod > 0 && conf.EventCh != nil ***REMOVED***
		c := &memberEventCoalescer***REMOVED***
	...
	***REMOVED***
	...
	if conf.UserCoalescePeriod > 0 && conf.UserQuiescentPeriod > 0 && conf.EventCh != nil ***REMOVED***
		c := &userEventCoalescer***REMOVED***
	...
	***REMOVED***
	...
	if !conf.DisableCoordinates ***REMOVED***
	...
	***REMOVED***
	...
	serf.broadcasts = &memberlist.TransmitLimitedQueue***REMOVED***
		NumNodes: func() int ***REMOVED***
			return len(serf.members)
		***REMOVED***,
		RetransmitMult: conf.MemberlistConfig.RetransmitMult,
	***REMOVED***
	serf.eventBroadcasts = &memberlist.TransmitLimitedQueue***REMOVED***
		NumNodes: func() int ***REMOVED***
			return len(serf.members)
		***REMOVED***,
		RetransmitMult: conf.MemberlistConfig.RetransmitMult,
	***REMOVED***
	serf.queryBroadcasts = &memberlist.TransmitLimitedQueue***REMOVED***
		NumNodes: func() int ***REMOVED***
			return len(serf.members)
		***REMOVED***,
		RetransmitMult: conf.MemberlistConfig.RetransmitMult,
	***REMOVED***
	...
	conf.MemberlistConfig.Events = &eventDelegate***REMOVED***serf: serf***REMOVED***
	conf.MemberlistConfig.Conflict = &conflictDelegate***REMOVED***serf: serf***REMOVED***
	conf.MemberlistConfig.Delegate = &delegate***REMOVED***serf: serf***REMOVED***
	...
	if !conf.DisableCoordinates ***REMOVED***
		conf.MemberlistConfig.Ping = &pingDelegate***REMOVED***serf: serf***REMOVED***
	***REMOVED***

	// Setup a merge delegate if necessary
	if conf.Merge != nil ***REMOVED***
		md := &mergeDelegate***REMOVED***serf: serf***REMOVED***
	...
	***REMOVED***
	...
	serf.keyManager = &KeyManager***REMOVED***serf: serf***REMOVED***
...
func (s *Serf) ProtocolVersion() uint8 ***REMOVED***
...
// starting in v0.2
func (s *Serf) UserEvent(name string, payload []byte, coalesce bool) error ***REMOVED***
	// Check the size limit
	if len(name)+len(payload) > UserEventSizeLimit ***REMOVED***
		return fmt.Errorf("user event exceeds limit of %d bytes", UserEventSizeLimit)
	***REMOVED***

	// Create a message
	msg := messageUserEvent***REMOVED***
	...
	***REMOVED***
	...
	if err != nil ***REMOVED***
		return err
	***REMOVED***
	s.eventBroadcasts.QueueBroadcast(&broadcast***REMOVED***
		msg: raw,
	***REMOVED***)
	return nil
***REMOVED***
...
func (s *Serf) Query(name string, payload []byte, params *QueryParam) (*QueryResponse, error) ***REMOVED***
	// Check that the latest protocol is in use
	if s.ProtocolVersion() < 4 ***REMOVED***
		return nil, FeatureNotSupported
	***REMOVED***

	// Provide default parameters if none given
	if params == nil ***REMOVED***
		params = s.DefaultQueryParams()
	***REMOVED*** else if params.Timeout == 0 ***REMOVED***
	...
	***REMOVED***
	...
	// Setup the flags
	var flags uint32
	if params.RequestAck ***REMOVED***
		flags |= queryFlagAck
	***REMOVED***

	// Create a message
	q := messageQuery***REMOVED***
	...
	***REMOVED***
	...
	// Check the size
	if len(raw) > s.config.QuerySizeLimit ***REMOVED***
	...
	***REMOVED***
	...
	// Start broadcasting the event
	s.queryBroadcasts.QueueBroadcast(&broadcast***REMOVED***
		msg: raw,
	***REMOVED***)
	return resp, nil
***REMOVED***
...
func (s *Serf) registerQueryResponse(timeout time.Duration, resp *QueryResponse) ***REMOVED***
	s.queryLock.Lock()
...
func (s *Serf) SetTags(tags map[string]string) error ***REMOVED***
	// Check that the meta data length is okay
	if len(s.encodeTags(tags)) > memberlist.MetaMaxSize ***REMOVED***
...
func (s *Serf) Join(existing []string, ignoreOld bool) (int, error) ***REMOVED***
	// Do a quick state check
	if s.State() != SerfAlive ***REMOVED***
	...
	***REMOVED***
	...
	if num > 0 ***REMOVED***
		...
		if err := s.broadcastJoin(s.clock.Time()); err != nil ***REMOVED***
			return num, err
		***REMOVED***
	***REMOVED***
...
func (s *Serf) broadcastJoin(ltime LamportTime) error ***REMOVED***
	// Construct message to update our lamport clock
	msg := messageJoin***REMOVED***
		LTime: ltime,
	...
	***REMOVED***
	...
	// Start broadcasting the update
	if err := s.broadcast(messageJoinType, &msg, nil); err != nil ***REMOVED***
...
// times.
func (s *Serf) Leave() error ***REMOVED***
	// Check the current state
	s.stateLock.Lock()
	if s.state == SerfLeft ***REMOVED***
		s.stateLock.Unlock()
		return nil
	***REMOVED*** else if s.state == SerfLeaving ***REMOVED***
		s.stateLock.Unlock()
		return fmt.Errorf("Leave already in progress")
	***REMOVED*** else if s.state == SerfShutdown ***REMOVED***
		s.stateLock.Unlock()
	...
	***REMOVED***
	...
	if s.snapshotter != nil ***REMOVED***
		s.snapshotter.Leave()
	***REMOVED***

	// Construct the message for the graceful leave
	msg := messageLeave***REMOVED***
		LTime: s.clock.Time(),
		Node:  s.config.NodeName,
	***REMOVED***
	s.clock.Increment()
	...
	// other node alive.
	if s.hasAliveMembers() ***REMOVED***
		notifyCh := make(chan struct***REMOVED******REMOVED***)
		if err := s.broadcast(messageLeaveType, &msg, notifyCh); err != nil ***REMOVED***
			return err
		***REMOVED***
	...
	***REMOVED***
	...
	s.stateLock.Lock()
	if s.state != SerfShutdown ***REMOVED***
		s.state = SerfLeft
	***REMOVED***
	s.stateLock.Unlock()
	return nil
...
// ourself.
func (s *Serf) hasAliveMembers() bool ***REMOVED***
	s.memberLock.RLock()
	defer s.memberLock.RUnlock()

	hasAlive := false
	for _, m := range s.members ***REMOVED***
		// Skip ourself, we want to know if OTHER members are alive
		if m.Name == s.config.NodeName ***REMOVED***
			continue
		***REMOVED***

		if m.Status == StatusAlive ***REMOVED***
			hasAlive = true
			break
		***REMOVED***
	***REMOVED***
...
func (s *Serf) LocalMember() Member ***REMOVED***
	s.memberLock.RLock()
	defer s.memberLock.RUnlock()
...
func (s *Serf) Members() []Member ***REMOVED***
	s.memberLock.RLock()
	defer s.memberLock.RUnlock()
...
// to this node.
func (s *Serf) RemoveFailedNode(node string) error ***REMOVED***
...
func (s *Serf) Shutdown() error ***REMOVED***
	s.stateLock.Lock()
...
// Serf to shutdown.
func (s *Serf) ShutdownCh() <-chan struct***REMOVED******REMOVED*** ***REMOVED***
	return s.shutdownCh
***REMOVED***

// Memberlist is used to get access to the underlying Memberlist instance
func (s *Serf) Memberlist() *memberlist.Memberlist ***REMOVED***
	return s.memberlist
***REMOVED***

// State is the current state of this Serf instance.
func (s *Serf) State() SerfState ***REMOVED***
	s.stateLock.Lock()
	defer s.stateLock.Unlock()
	return s.state
***REMOVED***
...
func (s *Serf) broadcast(t messageType, msg interface***REMOVED******REMOVED***, notify chan<- struct***REMOVED******REMOVED***) error ***REMOVED***
	raw, err := encodeMessage(t, msg)
	if err != nil ***REMOVED***
		return err
	***REMOVED***

	s.broadcasts.QueueBroadcast(&broadcast***REMOVED***
		msg:    raw,
		notify: notify,
	***REMOVED***)
	return nil
...
// from memberlist.
func (s *Serf) handleNodeJoin(n *memberlist.Node) ***REMOVED***
	s.memberLock.Lock()
	...
	if !ok ***REMOVED***
		...
		***REMOVED***
		...
		if join := recentIntent(s.recentJoin, n.Name); join != nil ***REMOVED***
			member.statusLTime = join.LTime
		***REMOVED***

		// Check if we have a leave intent
		if leave := recentIntent(s.recentLeave, n.Name); leave != nil ***REMOVED***
			if leave.LTime > member.statusLTime ***REMOVED***
	...
	***REMOVED*** else ***REMOVED***
		...
		member.leaveTime = time.Time***REMOVED******REMOVED***
	...
	***REMOVED***
	...
	if oldStatus == StatusFailed || oldStatus == StatusLeft ***REMOVED***
		s.failedMembers = removeOldMember(s.failedMembers, member.Name)
		s.leftMembers = removeOldMember(s.leftMembers, member.Name)
	***REMOVED***

	// Update some metrics
	metrics.IncrCounter([]string***REMOVED***"serf", "member", "join"***REMOVED***, 1)

	// Send an event along
	s.logger.Printf("[INFO] serf: EventMemberJoin: %s %s",
		member.Member.Name, member.Member.Addr)
	if s.config.EventCh != nil ***REMOVED***
		s.config.EventCh <- MemberEvent***REMOVED***
			Type:    EventMemberJoin,
			Members: []Member***REMOVED***member.Member***REMOVED***,
		***REMOVED***
	***REMOVED***
***REMOVED***
...
// from memberlist.
func (s *Serf) handleNodeLeave(n *memberlist.Node) ***REMOVED***
	s.memberLock.Lock()
	defer s.memberLock.Unlock()
	...
	***REMOVED***
	...
	if member.Status != StatusLeft ***REMOVED***
		event = EventMemberFailed
		eventStr = "EventMemberFailed"
	***REMOVED***

	// Update some metrics
	metrics.IncrCounter([]string***REMOVED***"serf", "member", member.Status.String()***REMOVED***, 1)
...
// has taken place
func (s *Serf) handleNodeUpdate(n *memberlist.Node) ***REMOVED***
	s.memberLock.Lock()
	defer s.memberLock.Unlock()
	...
	***REMOVED***
	...
	// Update some metrics
	metrics.IncrCounter([]string***REMOVED***"serf", "member", "update"***REMOVED***, 1)
...
func (s *Serf) handleNodeLeaveIntent(leaveMsg *messageLeave) bool ***REMOVED***
	...
	if !ok ***REMOVED***
		...
		if recentIntent(s.recentLeave, leaveMsg.Node) != nil ***REMOVED***
			return false
		***REMOVED***

		// We don't know this member so store it in a buffer for now
		s.recentLeave[s.recentLeaveIndex] = nodeIntent***REMOVED***
...
func (s *Serf) handleNodeJoinIntent(joinMsg *messageJoin) bool ***REMOVED***
	// Witness a potentially newer time
	...
	if !ok ***REMOVED***
		// If we've already seen this message don't rebroadcast
		if recentIntent(s.recentJoin, joinMsg.Node) != nil ***REMOVED***
			return false
		***REMOVED***

		// We don't know this member so store it in a buffer for now
		s.recentJoin[s.recentJoinIndex] = nodeIntent***REMOVED***LTime: joinMsg.LTime, Node: joinMsg.Node***REMOVED***
		s.recentJoinIndex = (s.recentJoinIndex + 1) % len(s.recentJoin)
		return true
	***REMOVED***

	// Check if this time is newer than what we have
	if joinMsg.LTime <= member.statusLTime ***REMOVED***
		return false
	***REMOVED***
	...
	if member.Status == StatusLeaving ***REMOVED***
		member.Status = StatusAlive
	***REMOVED***
	return true
***REMOVED***
...
func (s *Serf) handleUserEvent(eventMsg *messageUserEvent) bool ***REMOVED***
	// Witness a potentially newer time
	...
	if eventMsg.LTime < s.eventMinTime ***REMOVED***
		return false
	***REMOVED***
	...
	if curTime > LamportTime(len(s.eventBuffer)) &&
		eventMsg.LTime < curTime-LamportTime(len(s.eventBuffer)) ***REMOVED***
		s.logger.Printf(
	...
	***REMOVED***
	...
	userEvent := userEvent***REMOVED***Name: eventMsg.Name, Payload: eventMsg.Payload***REMOVED***
	if seen != nil && seen.LTime == eventMsg.LTime ***REMOVED***
		for _, previous := range seen.Events ***REMOVED***
			if previous.Equals(&userEvent) ***REMOVED***
				return false
			***REMOVED***
		***REMOVED***
	***REMOVED*** else ***REMOVED***
		seen = &userEvents***REMOVED***LTime: eventMsg.LTime***REMOVED***
	...
	***REMOVED***
	...
	// Update some metrics
	metrics.IncrCounter([]string***REMOVED***"serf", "events"***REMOVED***, 1)
	metrics.IncrCounter([]string***REMOVED***"serf", "events", eventMsg.Name***REMOVED***, 1)

	if s.config.EventCh != nil ***REMOVED***
		s.config.EventCh <- UserEvent***REMOVED***
...
// received. Returns if the message should be rebroadcast.
func (s *Serf) handleQuery(query *messageQuery) bool ***REMOVED***
	// Witness a potentially newer time
	...
	if query.LTime < s.queryMinTime ***REMOVED***
		return false
	***REMOVED***
	...
	if curTime > LamportTime(len(s.queryBuffer)) &&
		query.LTime < curTime-LamportTime(len(s.queryBuffer)) ***REMOVED***
		s.logger.Printf(
	...
	***REMOVED***
	...
	if seen != nil && seen.LTime == query.LTime ***REMOVED***
		for _, previous := range seen.QueryIDs ***REMOVED***
			if previous == query.ID ***REMOVED***
				// Seen this ID already
				return false
			***REMOVED***
		***REMOVED***
	***REMOVED*** else ***REMOVED***
		seen = &queries***REMOVED***LTime: query.LTime***REMOVED***
	...
	***REMOVED***
	...
	// Update some metrics
	metrics.IncrCounter([]string***REMOVED***"serf", "queries"***REMOVED***, 1)
	metrics.IncrCounter([]string***REMOVED***"serf", "queries", query.Name***REMOVED***, 1)

	// Check if we should rebroadcast, this may be disabled by a flag
	rebroadcast := true
	if query.NoBroadcast() ***REMOVED***
		rebroadcast = false
	***REMOVED***

	// Filter the query
	if !s.shouldProcessQuery(query.Filters) ***REMOVED***
	...
	***REMOVED***
	...
	if query.Ack() ***REMOVED***
		ack := messageQueryResponse***REMOVED***
			LTime: query.LTime,
			ID:    query.ID,
		...
		***REMOVED*** else ***REMOVED***
			addr := net.UDPAddr***REMOVED***IP: query.Addr, Port: int(query.Port)***REMOVED***
			if err := s.memberlist.SendTo(&addr, raw); err != nil ***REMOVED***
	...
	***REMOVED***
	...
	if s.config.EventCh != nil ***REMOVED***
		s.config.EventCh <- &Query***REMOVED***
			LTime:    query.LTime,
			Name:     query.Name,
...
// received.
func (s *Serf) handleQueryResponse(resp *messageQueryResponse) ***REMOVED***
	...
	***REMOVED***
	...
	if query.Finished() ***REMOVED***
		return
	***REMOVED***

	// Process each type of response
	if resp.Ack() ***REMOVED***
		metrics.IncrCounter([]string***REMOVED***"serf", "query_acks"***REMOVED***, 1)
		select ***REMOVED***
		...
		***REMOVED***
	***REMOVED*** else ***REMOVED***
		metrics.IncrCounter([]string***REMOVED***"serf", "query_responses"***REMOVED***, 1)
		select ***REMOVED***
		case query.respCh <- NodeResponse***REMOVED***From: resp.From, Payload: resp.Payload***REMOVED***:
		default:
...
func (s *Serf) handleNodeConflict(existing, other *memberlist.Node) ***REMOVED***
	// Log a basic warning if the node is not us...
	if existing.Name != s.config.NodeName ***REMOVED***
	...
	***REMOVED***
	...
	if s.config.EnableNameConflictResolution ***REMOVED***
...
func (s *Serf) resolveNodeConflict() ***REMOVED***
	// Get the local node
	local := s.memberlist.LocalNode()
	...
	***REMOVED***
	...
	for r := range respCh ***REMOVED***
		// Decode the response
		if len(r.Payload) < 1 || messageType(r.Payload[0]) != messageConflictResponseType ***REMOVED***
			s.logger.Printf("[ERR] serf: Invalid conflict query response type: %v", r.Payload)
			continue
		***REMOVED***
		var member Member
		if err := decodeMessage(r.Payload[1:], &member); err != nil ***REMOVED***
		...
		***REMOVED***
		...
		// Update the counters
		responses++
		if bytes.Equal(member.Addr, local.Addr) && member.Port == local.Port ***REMOVED***
			matching++
		***REMOVED***
	***REMOVED***
...
func (s *Serf) handleReap() ***REMOVED***
	for ***REMOVED***
		select ***REMOVED***
...
func (s *Serf) handleReconnect() ***REMOVED***
	for ***REMOVED***
		select ***REMOVED***
...
func (s *Serf) reap(old []*memberState, timeout time.Duration) []*memberState ***REMOVED***
	now := time.Now()
	n := len(old)
	for i := 0; i < n; i++ ***REMOVED***
		...
		if now.Sub(m.leaveTime) <= timeout ***REMOVED***
			continue
		***REMOVED***
		...
		if !s.config.DisableCoordinates ***REMOVED***
		...
		***REMOVED***
		...
		if s.config.EventCh != nil ***REMOVED***
			s.config.EventCh <- MemberEvent***REMOVED***
				...
				Members: []Member***REMOVED***m.Member***REMOVED***,
			***REMOVED***
		***REMOVED***
	***REMOVED***
...
func (s *Serf) reconnect() ***REMOVED***
	...
	***REMOVED***
	...
	// Format the addr
	addr := net.UDPAddr***REMOVED***IP: mem.Addr, Port: int(mem.Port)***REMOVED***
	s.logger.Printf("[INFO] serf: attempting reconnect to %v %s", mem.Name, addr.String())

	// Attempt to join at the memberlist level
	s.memberlist.Join([]string***REMOVED***addr.String()***REMOVED***)
***REMOVED***

// checkQueueDepth periodically checks the size of a queue to see if
// it is too large
func (s *Serf) checkQueueDepth(name string, queue *memberlist.TransmitLimitedQueue) ***REMOVED***
	for ***REMOVED***
		select ***REMOVED***
		case <-time.After(time.Second):
			numq := queue.NumQueued()
			metrics.AddSample([]string***REMOVED***"serf", "queue", name***REMOVED***, float32(numq))
			if numq >= s.config.QueueDepthWarning ***REMOVED***
				s.logger.Printf("[WARN] serf: %s queue depth: %d", name, numq)
			***REMOVED***
			if numq > s.config.MaxQueueDepth ***REMOVED***
...
// members.
func removeOldMember(old []*memberState, name string) []*memberState ***REMOVED***
	for i, m := range old ***REMOVED***
...
func recentIntent(recent []nodeIntent, node string) (intent *nodeIntent) ***REMOVED***
	for i := 0; i < len(recent); i++ ***REMOVED***
		// Break fast if we hit a zero entry
		if recent[i].LTime == 0 ***REMOVED***
			break
		***REMOVED***

		// Check for a node match
		if recent[i].Node == node ***REMOVED***
			// Take the most recent entry
			if intent == nil || recent[i].LTime > intent.LTime ***REMOVED***
				intent = &recent[i]
			***REMOVED***
		***REMOVED***
	***REMOVED***
...
func (s *Serf) handleRejoin(previous []*PreviousNode) ***REMOVED***
	for _, prev := range previous ***REMOVED***
		// Do not attempt to join ourself
		if prev.Name == s.config.NodeName ***REMOVED***
			continue
		***REMOVED***

		s.logger.Printf("[INFO] serf: Attempting re-join to previously known node: %s", prev)
		_, err := s.memberlist.Join([]string***REMOVED***prev.Addr***REMOVED***)
		if err == nil ***REMOVED***
...
func (s *Serf) encodeTags(tags map[string]string) []byte ***REMOVED***
	// Support role-only backwards compatibility
	if s.ProtocolVersion() < 3 ***REMOVED***
		role := tags["role"]
		return []byte(role)
	***REMOVED***
	...
	if err := enc.Encode(tags); err != nil ***REMOVED***
...
func (s *Serf) decodeTags(buf []byte) map[string]string ***REMOVED***
	tags := make(map[string]string)

	// Backwards compatibility mode
	if len(buf) == 0 || buf[0] != tagMagicByte ***REMOVED***
	...
	***REMOVED***
	...
	if err := dec.Decode(&tags); err != nil ***REMOVED***
...
func (s *Serf) Stats() map[string]string ***REMOVED***
	toString := func(v uint64) string ***REMOVED***
		return strconv.FormatUint(v, 10)
	***REMOVED***
	stats := map[string]string***REMOVED***
...
func (s *Serf) writeKeyringFile() error ***REMOVED***
	if len(s.config.KeyringFile) == 0 ***REMOVED***
		return nil
	***REMOVED***
	...
	for i, key := range keysRaw ***REMOVED***
	...
	***REMOVED***
	...
	if err = ioutil.WriteFile(s.config.KeyringFile, encodedKeys, 0600); err != nil ***REMOVED***
...
func (s *Serf) GetCoordinate() (*coordinate.Coordinate, error) ***REMOVED***
	if !s.config.DisableCoordinates ***REMOVED***
...
func (s *Serf) GetCachedCoordinate(name string) (coord *coordinate.Coordinate, ok bool) ***REMOVED***
	if !s.config.DisableCoordinates ***REMOVED***
		s.coordCacheLock.RLock()
		defer s.coordCacheLock.RUnlock()
		if coord, ok = s.coordCache[name]; ok ***REMOVED***
			return coord, true
		***REMOVED***
...
func (s *Serf) NumNodes() (numNodes int) ***REMOVED***
	s.memberLock.RLock()
