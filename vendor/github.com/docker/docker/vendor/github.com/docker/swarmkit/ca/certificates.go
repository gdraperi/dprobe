...
var BasicConstraintsOID = asn1.ObjectIdentifier***REMOVED***2, 5, 29, 19***REMOVED***
...
type recoverableErr struct ***REMOVED***
	err error
***REMOVED***

func (r recoverableErr) Error() string ***REMOVED***
	return r.err.Error()
***REMOVED***
...
// sign certificates.
var ErrNoValidSigner = recoverableErr***REMOVED***err: errors.New("no valid signer found")***REMOVED***
...
type x509UnknownAuthError struct ***REMOVED***
	error
...
func (rca *RootCA) Signer() (*LocalSigner, error) ***REMOVED***
	if rca.Pool == nil || rca.signer == nil || len(rca.signer.Cert) == 0 || rca.signer.Signer == nil ***REMOVED***
...
func (rca *RootCA) IssueAndSaveNewCertificates(kw KeyWriter, cn, ou, org string) (*tls.Certificate, *IssuerInfo, error) ***REMOVED***
	...
	***REMOVED***
	...
	if err != nil ***REMOVED*** // should never happen, since if ParseValidateAndSignCSR did not fail this root CA must have a signer
		return nil, nil, err
	***REMOVED***
	...
	***REMOVED***
	...
	if err := kw.Write(NormalizePEMs(certChain), key, nil); err != nil ***REMOVED***
		return nil, nil, err
	***REMOVED***

	return &tlsKeyPair, &IssuerInfo***REMOVED***
...
func (rca *RootCA) RequestAndSaveNewCertificates(ctx context.Context, kw KeyWriter, config CertificateRequestConfig) (*tls.Certificate, *IssuerInfo, error) ***REMOVED***
	...
	***REMOVED***
	...
	if _, ok := err.(x509.UnknownAuthorityError); ok ***REMOVED***
		if parsedCerts, parseErr := helpers.ParseCertificatesPEM(signedCert); parseErr == nil && len(parsedCerts) > 0 ***REMOVED***
			return nil, nil, x509UnknownAuthError***REMOVED***
				error:          err,
	...
	***REMOVED***
	...
	if err := kw.Write(NormalizePEMs(signedCert), key, kekUpdate); err != nil ***REMOVED***
		return nil, nil, err
	***REMOVED***
...
func (rca *RootCA) getKEKUpdate(ctx context.Context, leafCert *x509.Certificate, keypair tls.Certificate, config CertificateRequestConfig) (*KEKData, error) ***REMOVED***
	var managerRole bool
	for _, ou := range leafCert.Subject.OrganizationalUnit ***REMOVED***
		if ou == ManagerRole ***REMOVED***
			managerRole = true
			break
		***REMOVED***
	***REMOVED***

	if managerRole ***REMOVED***
		mtlsCreds := credentials.NewTLS(&tls.Config***REMOVED***ServerName: CARole, RootCAs: rca.Pool, Certificates: []tls.Certificate***REMOVED***keypair***REMOVED******REMOVED***)
		...
		***REMOVED***
		...
		defer cancel()
		response, err := client.GetUnlockKey(ctx, &api.GetUnlockKeyRequest***REMOVED******REMOVED***)
		if err != nil ***REMOVED***
			if grpc.Code(err) == codes.Unimplemented ***REMOVED*** // if the server does not support keks, return as if no encryption key was specified
				conn.Close(true)
				return &KEKData***REMOVED******REMOVED***, nil
			***REMOVED***
		...
		***REMOVED***
		conn.Close(true)
		return &KEKData***REMOVED***KEK: response.UnlockKey, Version: response.Version.Index***REMOVED***, nil
	***REMOVED***
...
func PrepareCSR(csrBytes []byte, cn, ou, org string) cfsigner.SignRequest ***REMOVED***
	// All managers get added the subject-alt-name of CA, so they can be
	// used for cert issuance.
	hosts := []string***REMOVED***ou, cn***REMOVED***
	if ou == ManagerRole ***REMOVED***
		hosts = append(hosts, CARole)
	***REMOVED***

	return cfsigner.SignRequest***REMOVED***
		Request: string(csrBytes),
		// OU is used for Authentication of the node type. The CN has the random
		// node ID.
		Subject: &cfsigner.Subject***REMOVED***CN: cn, Names: []cfcsr.Name***REMOVED******REMOVED***OU: ou, O: org***REMOVED******REMOVED******REMOVED***,
...
func (rca *RootCA) ParseValidateAndSignCSR(csrBytes []byte, cn, ou, org string) ([]byte, error) ***REMOVED***
...
func (rca *RootCA) CrossSignCACertificate(otherCAPEM []byte) ([]byte, error) ***REMOVED***
	signer, err := rca.Signer()
	if err != nil ***REMOVED***
		return nil, err
	***REMOVED***
	...
	***REMOVED***
	...
	return pem.EncodeToMemory(&pem.Block***REMOVED***
		Type:  "CERTIFICATE",
		Bytes: derBytes,
	***REMOVED***), nil
***REMOVED***

func validateSignatureAlgorithm(cert *x509.Certificate) error ***REMOVED***
	switch cert.SignatureAlgorithm ***REMOVED***
...
// without a signer.
func NewRootCA(rootCertBytes, signCertBytes, signKeyBytes []byte, certExpiry time.Duration, intermediates []byte) (RootCA, error) ***REMOVED***
	// Parse all the certificates in the cert bundle
	parsedCerts, err := helpers.ParseCertificatesPEM(rootCertBytes)
	if err != nil ***REMOVED***
		return RootCA***REMOVED******REMOVED***, errors.Wrap(err, "invalid root certificates")
	***REMOVED***
	// Check to see if we have at least one valid cert
	if len(parsedCerts) < 1 ***REMOVED***
		return RootCA***REMOVED******REMOVED***, errors.New("no valid root CA certificates found")
	***REMOVED***

	// Create a Pool with all of the certificates found
	pool := x509.NewCertPool()
	for _, cert := range parsedCerts ***REMOVED***
		if err := validateSignatureAlgorithm(cert); err != nil ***REMOVED***
		...
		***REMOVED***
		...
		if _, err := cert.Verify(x509.VerifyOptions***REMOVED***Roots: selfpool***REMOVED***); err != nil ***REMOVED***
			return RootCA***REMOVED******REMOVED***, errors.Wrap(err, "error while validating Root CA Certificate")
		***REMOVED***
		pool.AddCert(cert)
	***REMOVED***
	...
	if len(intermediates) > 0 ***REMOVED***
		parsedIntermediates, _, err = ValidateCertChain(pool, intermediates, false)
		if err != nil ***REMOVED***
			return RootCA***REMOVED******REMOVED***, errors.Wrap(err, "invalid intermediate chain")
		***REMOVED***
		intermediatePool = x509.NewCertPool()
		for _, cert := range parsedIntermediates ***REMOVED***
			intermediatePool.AddCert(cert)
		***REMOVED***
	***REMOVED***

	var localSigner *LocalSigner
	if len(signKeyBytes) != 0 || len(signCertBytes) != 0 ***REMOVED***
		...
		***REMOVED***
		...
		if len(parsedIntermediates) > 0 && (!bytes.Equal(parsedIntermediates[0].RawSubject, localSigner.parsedCert.RawSubject) ||
			!bytes.Equal(parsedIntermediates[0].RawSubjectPublicKeyInfo, localSigner.parsedCert.RawSubjectPublicKeyInfo)) ***REMOVED***
			return RootCA***REMOVED******REMOVED***, errors.New(
				"invalid intermediate chain - the first intermediate must have the same subject and public key as the signing cert")
		***REMOVED***
	***REMOVED***

	return RootCA***REMOVED***signer: localSigner, Intermediates: intermediates, Digest: digest, Certs: rootCertBytes, Pool: pool***REMOVED***, nil
***REMOVED***
...
func ValidateCertChain(rootPool *x509.CertPool, certs []byte, allowExpired bool) ([]*x509.Certificate, [][]*x509.Certificate, error) ***REMOVED***
	// Parse all the certificates in the cert bundle
	parsedCerts, err := helpers.ParseCertificatesPEM(certs)
	if err != nil ***REMOVED***
		return nil, nil, err
	***REMOVED***
	if len(parsedCerts) == 0 ***REMOVED***
	...
	***REMOVED***
	...
	var intermediatePool *x509.CertPool
	for i, cert := range parsedCerts ***REMOVED***
		...
		if now.Before(cert.NotBefore) ***REMOVED***
			return nil, nil, errors.Wrapf(
				x509.CertificateInvalidError***REMOVED***
					Cert:   cert,
					Reason: x509.Expired,
				***REMOVED***,
		...
		***REMOVED***
		if !allowExpired && now.After(cert.NotAfter) ***REMOVED***
			return nil, nil, errors.Wrapf(
				x509.CertificateInvalidError***REMOVED***
					Cert:   cert,
					Reason: x509.Expired,
		...
		***REMOVED***
		...
		if i > 0 ***REMOVED***
			...
			if err := prevCert.CheckSignatureFrom(cert); err != nil ***REMOVED***
	...
	***REMOVED***
	...
	verifyOpts := x509.VerifyOptions***REMOVED***
		Roots:         rootPool,
	...
	***REMOVED***
	...
	if allowExpired ***REMOVED***
		verifyOpts.CurrentTime = parsedCerts[0].NotAfter.Add(time.Hour)
		for _, cert := range parsedCerts ***REMOVED***
			if !cert.NotAfter.Before(verifyOpts.CurrentTime) ***REMOVED***
				continue
			***REMOVED***
			...
			***REMOVED***
		***REMOVED***
		if invalid, ok := err.(x509.CertificateInvalidError); ok && invalid.Reason == x509.Expired ***REMOVED***
...
func newLocalSigner(keyBytes, certBytes []byte, certExpiry time.Duration, rootPool, intermediatePool *x509.CertPool) (*LocalSigner, error) ***REMOVED***
	if len(keyBytes) == 0 || len(certBytes) == 0 ***REMOVED***
	...
	***REMOVED***
	if err := validateSignatureAlgorithm(parsedCerts[0]); err != nil ***REMOVED***
		return nil, err
	***REMOVED***
	opts := x509.VerifyOptions***REMOVED***
		...
		Intermediates: intermediatePool,
	***REMOVED***
	if _, err := parsedCerts[0].Verify(opts); err != nil ***REMOVED***
	...
	***REMOVED***
	...
	if passphraseStr = os.Getenv(PassphraseENVVar); passphraseStr != "" ***REMOVED***
		passphrase = []byte(passphraseStr)
	***REMOVED***

	if p := os.Getenv(PassphraseENVVarPrev); p != "" ***REMOVED***
	...
	***REMOVED***
	...
	if err := ensureCertKeyMatch(parsedCerts[0], priv.Public()); err != nil ***REMOVED***
		return nil, err
	***REMOVED***
	...
	***REMOVED***
	...
	if passphraseStr != "" && !x509.IsEncryptedPEMBlock(keyBlock) ***REMOVED***
	...
	***REMOVED***
	...
	return &LocalSigner***REMOVED***Cert: certBytes, Key: keyBytes, Signer: signer, parsedCert: parsedCerts[0], cryptoSigner: priv***REMOVED***, nil
***REMOVED***

func ensureCertKeyMatch(cert *x509.Certificate, key crypto.PublicKey) error ***REMOVED***
	switch certPub := cert.PublicKey.(type) ***REMOVED***
	case *rsa.PublicKey:
		if certPub.N.BitLen() < 2048 || certPub.E == 1 ***REMOVED***
			return errors.New("unsupported RSA key parameters")
		***REMOVED***
		rsaKey, ok := key.(*rsa.PublicKey)
		if ok && certPub.E == rsaKey.E && certPub.N.Cmp(rsaKey.N) == 0 ***REMOVED***
			return nil
		***REMOVED***
	case *ecdsa.PublicKey:
		switch certPub.Curve ***REMOVED***
		...
		***REMOVED***
		...
		if ok && certPub.X.Cmp(ecKey.X) == 0 && certPub.Y.Cmp(ecKey.Y) == 0 ***REMOVED***
			return nil
		***REMOVED***
	default:
...
func GetLocalRootCA(paths CertPaths) (RootCA, error) ***REMOVED***
...
func getGRPCConnection(creds credentials.TransportCredentials, connBroker *connectionbroker.Broker, forceRemote bool) (*connectionbroker.Conn, error) ***REMOVED***
	dialOpts := []grpc.DialOption***REMOVED***
		grpc.WithTransportCredentials(creds),
		grpc.WithTimeout(5 * time.Second),
		grpc.WithBackoffMaxDelay(5 * time.Second),
	***REMOVED***
	if forceRemote ***REMOVED***
...
func GetRemoteCA(ctx context.Context, d digest.Digest, connBroker *connectionbroker.Broker) (RootCA, error) ***REMOVED***
	...
	***REMOVED***
	...
	defer func() ***REMOVED***
		conn.Close(err == nil)
	***REMOVED***()
	response, err := client.GetRootCACertificate(ctx, &api.GetRootCACertificateRequest***REMOVED******REMOVED***)
	if err != nil ***REMOVED***
		return RootCA***REMOVED******REMOVED***, err
	***REMOVED***
	...
	if d != "" ***REMOVED***
		verifier := d.Verifier()
		if err != nil ***REMOVED***
			return RootCA***REMOVED******REMOVED***, errors.Wrap(err, "unexpected error getting digest verifier")
		***REMOVED***

		io.Copy(verifier, bytes.NewReader(response.Certificate))

		if !verifier.Verified() ***REMOVED***
			return RootCA***REMOVED******REMOVED***, errors.Errorf("remote CA does not match fingerprint. Expected: %s", d.Hex())
		***REMOVED***
	***REMOVED***
...
func CreateRootCA(rootCN string) (RootCA, error) ***REMOVED***
	// Create a simple CSR for the CA using the default CA validator and policy
	req := cfcsr.CertificateRequest***REMOVED***
		CN:         rootCN,
		KeyRequest: &cfcsr.BasicKeyRequest***REMOVED***A: RootKeyAlgo, S: RootKeySize***REMOVED***,
		CA:         &cfcsr.CAConfig***REMOVED***Expiry: RootCAExpiration***REMOVED***,
	***REMOVED***
...
func GetRemoteSignedCertificate(ctx context.Context, csr []byte, rootCAPool *x509.CertPool, config CertificateRequestConfig) ([]byte, error) ***REMOVED***
	...
	***REMOVED***
	...
	if creds == nil ***REMOVED***
		...
		creds = credentials.NewTLS(&tls.Config***REMOVED***ServerName: CARole, RootCAs: rootCAPool***REMOVED***)
	***REMOVED***
	...
	***REMOVED***
	...
	issueRequest := &api.IssueNodeCertificateRequest***REMOVED***CSR: csr, Token: config.Token, Availability: config.Availability***REMOVED***
	...
	***REMOVED***
	...
	statusRequest := &api.NodeCertificateStatusRequest***REMOVED***NodeID: issueResponse.NodeID***REMOVED***
	expBackoff := events.NewExponentialBackoff(events.ExponentialBackoffConfig***REMOVED***
		Base:   time.Second,
		Factor: time.Second,
		Max:    30 * time.Second,
	***REMOVED***)
	...
	for ***REMOVED***
		...
		if config.NodeCertificateStatusRequestTimeout > 0 ***REMOVED***
		...
		***REMOVED***
		...
		case err == nil && (statusResponse.Status.State == api.IssuanceStateIssued || statusResponse.Status.State == api.IssuanceStateRotate):
			if statusResponse.Certificate == nil ***REMOVED***
				conn.Close(false)
			...
			***REMOVED***
			...
			// current request.
			if bytes.Equal(statusResponse.Certificate.CSR, csr) ***REMOVED***
				conn.Close(true)
...
func readCertValidity(kr KeyReader) (time.Time, time.Time, error) ***REMOVED***
...
func SaveRootCA(rootCA RootCA, paths CertPaths) error ***REMOVED***
...
func GenerateNewCSR() ([]byte, []byte, error) ***REMOVED***
	req := &cfcsr.CertificateRequest***REMOVED***
...
func EncryptECPrivateKey(key []byte, passphraseStr string) ([]byte, error) ***REMOVED***
	passphrase := []byte(passphraseStr)
...
func NormalizePEMs(certs []byte) []byte ***REMOVED***
	var (
		results  []byte
		pemBlock *pem.Block
	)
