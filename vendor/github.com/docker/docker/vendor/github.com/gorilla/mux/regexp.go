...
func newRouteRegexp(tpl string, matchHost, matchPrefix, matchQuery, strictSlash bool) (*routeRegexp, error) ***REMOVED***
...
func extractVars(input string, matches []int, names []string, output map[string]string) ***REMOVED***
	matchesCount := 0
	prevEnd := -1
	for i := 2; i < len(matches) && matchesCount < len(names); i += 2 ***REMOVED***
		if prevEnd < matches[i+1] ***REMOVED***
