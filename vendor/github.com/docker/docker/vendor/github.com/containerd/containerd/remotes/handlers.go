...
func MakeRefKey(ctx context.Context, desc ocispec.Descriptor) string ***REMOVED***
...
// recursive fetch.
func FetchHandler(ingester content.Ingester, fetcher Fetcher) images.HandlerFunc ***REMOVED***
...
func fetch(ctx context.Context, ingester content.Ingester, fetcher Fetcher, desc ocispec.Descriptor) error ***REMOVED***
	...
	for ***REMOVED***
		...
		if err != nil ***REMOVED***
			if errdefs.IsAlreadyExists(err) ***REMOVED***
				return nil
			***REMOVED*** else if !errdefs.IsUnavailable(err) ***REMOVED***
				return err
			***REMOVED***
...
func commitOpts(desc ocispec.Descriptor, r io.Reader) (io.Reader, []content.Opt) ***REMOVED***
	var childrenF func(r io.Reader) ([]ocispec.Descriptor, error)

	// TODO(AkihiroSuda): use images/oci.GetChildrenDescriptors?
	switch desc.MediaType ***REMOVED***
	case images.MediaTypeDockerSchema2Manifest, ocispec.MediaTypeImageManifest:
		childrenF = func(r io.Reader) ([]ocispec.Descriptor, error) ***REMOVED***
			var (
				manifest ocispec.Manifest
				decoder  = json.NewDecoder(r)
			)
			if err := decoder.Decode(&manifest); err != nil ***REMOVED***
				return nil, err
			***REMOVED***

			return append([]ocispec.Descriptor***REMOVED***manifest.Config***REMOVED***, manifest.Layers...), nil
		***REMOVED***
	case images.MediaTypeDockerSchema2ManifestList, ocispec.MediaTypeImageIndex:
		childrenF = func(r io.Reader) ([]ocispec.Descriptor, error) ***REMOVED***
			var (
			...
			if err := decoder.Decode(&index); err != nil ***REMOVED***
				return nil, err
			***REMOVED***
	...
	***REMOVED***()
	...
	opt := func(info *content.Info) error ***REMOVED***
		err := <-errC
		if err != nil ***REMOVED***
		...
		***REMOVED***
		...
		if len(children) > 0 ***REMOVED***
			if info.Labels == nil ***REMOVED***
				info.Labels = map[string]string***REMOVED******REMOVED***
			***REMOVED***
			for i, ch := range children ***REMOVED***
	...
	***REMOVED***
	...
	return io.TeeReader(r, pw), []content.Opt***REMOVED***opt***REMOVED***
***REMOVED***

// PushHandler returns a handler that will push all content from the provider
// using a writer from the pusher.
func PushHandler(provider content.Provider, pusher Pusher) images.HandlerFunc ***REMOVED***
...
func push(ctx context.Context, provider content.Provider, pusher Pusher, desc ocispec.Descriptor) error ***REMOVED***
