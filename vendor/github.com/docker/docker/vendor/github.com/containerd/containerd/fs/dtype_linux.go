...
func SupportsDType(path string) (bool, error) ***REMOVED***
	...
	***REMOVED***
	...
	visited := 0
	supportsDType := true
	fn := func(ent *syscall.Dirent) bool ***REMOVED***
		visited++
		if ent.Type == syscall.DT_UNKNOWN ***REMOVED***
			supportsDType = false
			// stop iteration
			return true
		***REMOVED***
...
func iterateReadDir(path string, fn func(*syscall.Dirent) bool) error ***REMOVED***
	d, err := os.Open(path)
	if err != nil ***REMOVED***
		return err
	***REMOVED***
