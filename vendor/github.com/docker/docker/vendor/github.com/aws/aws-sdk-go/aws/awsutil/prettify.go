...
func Prettify(i interface***REMOVED******REMOVED***) string ***REMOVED***
...
func prettify(v reflect.Value, indent int, buf *bytes.Buffer) ***REMOVED***
	...
	***REMOVED***
	...
	case reflect.Struct:
		...
		if strtype == "time.Time" ***REMOVED***
			fmt.Fprintf(buf, "%s", v.Interface())
			break
		***REMOVED*** else if strings.HasPrefix(strtype, "io.") ***REMOVED***
		...
		***REMOVED***
		...
		for i := 0; i < v.Type().NumField(); i++ ***REMOVED***
			...
			***REMOVED***
			if (f.Kind() == reflect.Ptr || f.Kind() == reflect.Slice || f.Kind() == reflect.Map) && f.IsNil() ***REMOVED***
