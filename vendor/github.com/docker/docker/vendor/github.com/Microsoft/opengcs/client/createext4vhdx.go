...
// as well.
func (config *Config) dismount(file string) error ***REMOVED***
...
func (config *Config) CreateExt4Vhdx(destFile string, sizeGB uint32, cacheFile string) error ***REMOVED***
	// Smallest we can accept is the default sandbox size as we can't size down, only expand.
	if sizeGB < DefaultVhdxSizeGB ***REMOVED***
	...
	***REMOVED***
	...
	if cacheFile != "" && sizeGB == DefaultVhdxSizeGB ***REMOVED***
		if _, err := os.Stat(cacheFile); err == nil ***REMOVED***
			if err := CopyFile(cacheFile, destFile, false); err != nil ***REMOVED***
	...
	***REMOVED***
	...
	// Create the VHDX
	if err := winio.CreateVhdx(destFile, sizeGB, defaultVhdxBlockSizeMB); err != nil ***REMOVED***
	...
	***REMOVED***
	...
	if err := config.HotAddVhd(destFile, "", false, false); err != nil ***REMOVED***
	...
	***REMOVED***
	...
	controller := -1
	lun := -1
	for controllerNumber, controllerElement := range mvdControllers ***REMOVED***
		for diskNumber, diskElement := range controllerElement.MappedVirtualDisks ***REMOVED***
			if diskElement.HostPath == destFile ***REMOVED***
	...
	***REMOVED***
	...
	if err := config.dismount(destFile); err != nil ***REMOVED***
		return fmt.Errorf("failed to hot-remove: %s", err)
	***REMOVED***

	// Populate the cache.
	if cacheFile != "" && (sizeGB == DefaultVhdxSizeGB) ***REMOVED***
		if err := CopyFile(destFile, cacheFile, true); err != nil ***REMOVED***
