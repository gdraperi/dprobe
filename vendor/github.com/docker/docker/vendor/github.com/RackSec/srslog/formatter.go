...
func DefaultFormatter(p Priority, hostname, tag, content string) string ***REMOVED***
...
func UnixFormatter(p Priority, hostname, tag, content string) string ***REMOVED***
...
func RFC3164Formatter(p Priority, hostname, tag, content string) string ***REMOVED***
	timestamp := time.Now().Format(time.Stamp)
	msg := fmt.Sprintf("<%d>%s %s %s[%d]: %s",
		p, timestamp, hostname, tag, os.Getpid(), content)
	return msg
...
func RFC5424Formatter(p Priority, hostname, tag, content string) string ***REMOVED***
	timestamp := time.Now().Format(time.RFC3339)
	pid := os.Getpid()
	appName := os.Args[0]
