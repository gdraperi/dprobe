...
func (f *FlagSet) SetNormalizeFunc(n func(f *FlagSet, name string) NormalizedName) ***REMOVED***
	...
	for k, v := range f.formal ***REMOVED***
		delete(f.formal, k)
...
func (f *FlagSet) VisitAll(fn func(*Flag)) ***REMOVED***
	for _, flag := range sortFlags(f.formal) ***REMOVED***
		fn(flag)
	***REMOVED***
***REMOVED***
...
func (f *FlagSet) Visit(fn func(*Flag)) ***REMOVED***
	for _, flag := range sortFlags(f.actual) ***REMOVED***
		fn(flag)
	***REMOVED***
***REMOVED***
...
func (f *FlagSet) setFlag(flag *Flag, value string, origArg string) error ***REMOVED***
	if err := flag.Value.Set(value); err != nil ***REMOVED***
	...
	***REMOVED***
	...
	if len(flag.Deprecated) > 0 ***REMOVED***
		fmt.Fprintf(os.Stderr, "Flag --%s has been deprecated, %s\n", flag.Name, flag.Deprecated)
	***REMOVED***
	if len(flag.ShorthandDeprecated) > 0 && containsShorthand(origArg, flag.Shorthand) ***REMOVED***
		fmt.Fprintf(os.Stderr, "Flag shorthand -%s has been deprecated, %s\n", flag.Shorthand, flag.ShorthandDeprecated)
	***REMOVED***
	return nil
***REMOVED***

func containsShorthand(arg, shorthand string) bool ***REMOVED***
...
func (f *FlagSet) parseLongArg(s string, args []string, fn parseFunc) (a []string, err error) ***REMOVED***
	...
	***REMOVED***
	...
	if len(split) == 2 ***REMOVED***
		// '--flag=arg'
		value = split[1]
	***REMOVED*** else if len(flag.NoOptDefVal) > 0 ***REMOVED***
...
func (f *FlagSet) Parse(arguments []string) error ***REMOVED***
	...
	assign := func(flag *Flag, value, origArg string) error ***REMOVED***
