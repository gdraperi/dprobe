...
func (sc *serverConn) condlogf(err error, format string, args ...interface***REMOVED******REMOVED***) ***REMOVED***
	if err == nil ***REMOVED***
		return
	***REMOVED***
	if err == io.EOF || err == io.ErrUnexpectedEOF || isClosedConnError(err) ***REMOVED***
	...
	***REMOVED***
	...
	for ***REMOVED***
		...
		***REMOVED***
		...
		if sc.inGoAway && sc.curOpenStreams() == 0 && !sc.needToSendGoAway && !sc.writingFrame ***REMOVED***
			return
		***REMOVED***
	***REMOVED***
***REMOVED***
...
func (sc *serverConn) goAwayIn(code ErrCode, forceCloseIn time.Duration) ***REMOVED***
	sc.serveG.check()
	if sc.inGoAway ***REMOVED***
		return
	***REMOVED***
	if forceCloseIn != 0 ***REMOVED***
