...
var isQueryTests = []struct ***REMOVED***
	msg             string
	expectedVersion int
***REMOVED******REMOVED***
	***REMOVED***"foo", 0***REMOVED***,
	***REMOVED***"?OtR", 0***REMOVED***,
	***REMOVED***"?OtR?", 0***REMOVED***,
	***REMOVED***"?OTR?", 0***REMOVED***,
	***REMOVED***"?OTRv?", 0***REMOVED***,
	***REMOVED***"?OTRv1?", 0***REMOVED***,
	***REMOVED***"?OTR?v1?", 0***REMOVED***,
	***REMOVED***"?OTR?v?", 0***REMOVED***,
	***REMOVED***"?OTR?v2?", 2***REMOVED***,
	***REMOVED***"?OTRv2?", 2***REMOVED***,
	***REMOVED***"?OTRv23?", 2***REMOVED***,
	***REMOVED***"?OTRv23 ?", 0***REMOVED***,
***REMOVED***

func TestIsQuery(t *testing.T) ***REMOVED***
	for i, test := range isQueryTests ***REMOVED***
		version := isQuery([]byte(test.msg))
		if version != test.expectedVersion ***REMOVED***
...
func TestKeySerialization(t *testing.T) ***REMOVED***
	var priv PrivateKey
	...
	***REMOVED***
	...
	if !bytes.Equal(alicePrivateKey, out) ***REMOVED***
	...
	***REMOVED***
	...
	if !bytes.Equal(aliceFingerprint, fingerprint) ***REMOVED***
...
func TestParseLibOTRPrivateKey(t *testing.T) ***REMOVED***
	var priv PrivateKey

	if !priv.Import([]byte(libOTRPrivateKey)) ***REMOVED***
...
func setupConversation(t *testing.T) (alice, bob *Conversation) ***REMOVED***
	alicePrivateKey, _ := hex.DecodeString(alicePrivateKeyHex)
...
func performHandshake(t *testing.T, alice, bob *Conversation) ***REMOVED***
	...
	for round := 0; len(alicesMessage) > 0 || len(bobsMessage) > 0; round++ ***REMOVED***
		bobsMessage = nil
		for i, msg := range alicesMessage ***REMOVED***
			...
			***REMOVED***
			if len(bobsMessage) > 0 && i != len(alicesMessage)-1 ***REMOVED***
				t.Errorf("Bob produced output while processing a fragment, round %d, message %d", round, i)
			***REMOVED***
		***REMOVED***

		alicesMessage = nil
		for i, msg := range bobsMessage ***REMOVED***
...
func roundTrip(t *testing.T, alice, bob *Conversation, message []byte, macKeyCheck int) ***REMOVED***
	alicesMessage, err := alice.Send(message)
	if err != nil ***REMOVED***
		t.Errorf("Error from Alice sending message: %s", err)
	***REMOVED***

	if len(alice.oldMACs) != 0 ***REMOVED***
	...
	***REMOVED***
	...
	for i, msg := range alicesMessage ***REMOVED***
		...
		***REMOVED***
		if len(out) > 0 ***REMOVED***
			if i != len(alicesMessage)-1 ***REMOVED***
	...
	***REMOVED***
	...
	for i, msg := range bobsMessage ***REMOVED***
		...
		***REMOVED***
		if len(out) > 0 ***REMOVED***
			if i != len(bobsMessage)-1 ***REMOVED***
...
func TestGoodSMP(t *testing.T) ***REMOVED***
	...
	for round := 0; len(alicesMessages) > 0 || len(bobsMessages) > 0; round++ ***REMOVED***
		bobsMessages = bobsMessages[:0]
		for i, msg := range alicesMessages ***REMOVED***
			out, bobComplete, err = bob.processSMP(msg)
			if err != nil ***REMOVED***
				t.Errorf("Error from Bob in round %d: %s", round, err)
			***REMOVED***
			if bobComplete && i != len(alicesMessages)-1 ***REMOVED***
...
func TestBadSMP(t *testing.T) ***REMOVED***
	...
	for round := 0; len(alicesMessages) > 0 || len(bobsMessages) > 0; round++ ***REMOVED***
		bobsMessages = bobsMessages[:0]
		for _, msg := range alicesMessages ***REMOVED***
		...
		***REMOVED***
		...
		alicesMessages = alicesMessages[:0]
		for _, msg := range bobsMessages ***REMOVED***
...
func TestRehandshaking(t *testing.T) ***REMOVED***
	alice, bob := setupConversation(t)
...
func TestAgainstLibOTR(t *testing.T) ***REMOVED***
	...
	for ***REMOVED***
		...
		***REMOVED***
		...
		if err != nil ***REMOVED***
			t.Fatal(err)
		***REMOVED***
		for _, msg := range alicesMessage ***REMOVED***
			out.Write(msg)
			out.Write([]byte("\n"))
		***REMOVED***
		if change == NewKeys ***REMOVED***
		...
		***REMOVED***
		if len(text) > 0 ***REMOVED***
			if !bytes.Equal(text, expectedText) ***REMOVED***
