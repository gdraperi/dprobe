...
func NewCipher(key []byte) (*Cipher, error) ***REMOVED***
	...
	if keylen != 16 && keylen != 24 && keylen != 32 ***REMOVED***
	...
	***REMOVED***
	...
	for i := 0; i < k; i++ ***REMOVED***
		...
		for j, rsRow := range rs ***REMOVED***
			for k, rsVal := range rsRow ***REMOVED***
	...
	***REMOVED***
	...
	c := new(Cipher)
	var tmp [4]byte
	for i := byte(0); i < 20; i++ ***REMOVED***
		// A = h(p * 2x, Me)
		for j := range tmp ***REMOVED***
			tmp[j] = 2 * i
		***REMOVED***
		A := h(tmp[:], key, 0)
...
func store32l(dst []byte, src uint32) ***REMOVED***
	dst[0] = byte(src)
	dst[1] = byte(src >> 8)
	dst[2] = byte(src >> 16)
	dst[3] = byte(src >> 24)
...
func load32l(src []byte) uint32 ***REMOVED***
	return uint32(src[0]) | uint32(src[1])<<8 | uint32(src[2])<<16 | uint32(src[3])<<24
***REMOVED***

// rol returns x after a left circular rotation of y bits.
func rol(x, y uint32) uint32 ***REMOVED***
	return (x << (y & 31)) | (x >> (32 - (y & 31)))
***REMOVED***

// ror returns x after a right circular rotation of y bits.
func ror(x, y uint32) uint32 ***REMOVED***
	return (x >> (y & 31)) | (x << (32 - (y & 31)))
***REMOVED***

// The RS matrix. See [TWOFISH] 4.3
var rs = [4][8]byte***REMOVED***
	***REMOVED***0x01, 0xA4, 0x55, 0x87, 0x5A, 0x58, 0xDB, 0x9E***REMOVED***,
	***REMOVED***0xA4, 0x56, 0x82, 0xF3, 0x1E, 0xC6, 0x68, 0xE5***REMOVED***,
	***REMOVED***0x02, 0xA1, 0xFC, 0xC1, 0x47, 0xAE, 0x3D, 0x19***REMOVED***,
	***REMOVED***0xA4, 0x55, 0x87, 0x5A, 0x58, 0xDB, 0x9E, 0x03***REMOVED***,
***REMOVED***

// sbox tables
var sbox = [2][256]byte***REMOVED***
	***REMOVED***
...
func gfMult(a, b byte, p uint32) byte ***REMOVED***
	B := [2]uint32***REMOVED***0, uint32(b)***REMOVED***
	P := [2]uint32***REMOVED***0, p***REMOVED***
	var result uint32
...
// mdsColumnMult calculates y***REMOVED***col***REMOVED*** where [y0 y1 y2 y3] = MDS · [x0]
func mdsColumnMult(in byte, col int) uint32 ***REMOVED***
	mul01 := in
...
func h(in, key []byte, offset int) uint32 ***REMOVED***
	var y [4]byte
	for x := range y ***REMOVED***
		y[x] = in[x]
	***REMOVED***
	switch len(key) / 8 ***REMOVED***
	case 4:
...
func (c *Cipher) Encrypt(dst, src []byte) ***REMOVED***
	S1 := c.s[0]
	S2 := c.s[1]
	S3 := c.s[2]
	S4 := c.s[3]
...
func (c *Cipher) Decrypt(dst, src []byte) ***REMOVED***
	S1 := c.s[0]
	S2 := c.s[1]
	S3 := c.s[2]
	S4 := c.s[3]
