...
var bad = []testVector***REMOVED***
	***REMOVED***"p", "s", 0, 1, 1, nil***REMOVED***,                    // N == 0
	***REMOVED***"p", "s", 1, 1, 1, nil***REMOVED***,                    // N == 1
	***REMOVED***"p", "s", 7, 8, 1, nil***REMOVED***,                    // N is not power of 2
	***REMOVED***"p", "s", 16, maxInt / 2, maxInt / 2, nil***REMOVED***, // p * r too large
***REMOVED***

func TestKey(t *testing.T) ***REMOVED***
	for i, v := range good ***REMOVED***
		k, err := Key([]byte(v.password), []byte(v.salt), v.N, v.r, v.p, len(v.output))
		if err != nil ***REMOVED***
			t.Errorf("%d: got unexpected error: %s", i, err)
		***REMOVED***
		if !bytes.Equal(k, v.output) ***REMOVED***
			t.Errorf("%d: expected %x, got %x", i, v.output, k)
		***REMOVED***
	***REMOVED***
	for i, v := range bad ***REMOVED***
...
func BenchmarkKey(b *testing.B) ***REMOVED***
