...
func TestSalsa20(t *testing.T) ***REMOVED***
	var inBuf, outBuf []byte
	var key [32]byte

	for i, test := range testVectors ***REMOVED***
		if test.numBytes%64 != 0 ***REMOVED***
			t.Errorf("#%d: numBytes is not a multiple of 64", i)
			continue
		***REMOVED***

		if test.numBytes > len(inBuf) ***REMOVED***
		...
		***REMOVED***
		...
		if !bytes.Equal(xor[:], test.xor) ***REMOVED***
			t.Errorf("#%d: bad result", i)
		***REMOVED***
	***REMOVED***
***REMOVED***

var xSalsa20TestData = []struct ***REMOVED***
...
***REMOVED******REMOVED***
	***REMOVED***
		...
		[]byte***REMOVED***0x00, 0x2d, 0x45, 0x13, 0x84, 0x3f, 0xc2, 0x40, 0xc4, 0x01, 0xe5, 0x41***REMOVED***,
	***REMOVED***,
	***REMOVED***
		make([]byte, 64),
		[]byte("24-byte nonce for xsalsa"),
		[]byte("this is 32-byte key for xsalsa20"),
		[]byte***REMOVED***0x48, 0x48, 0x29, 0x7f, 0xeb, 0x1f, 0xb5, 0x2f, 0xb6,
			...
			0x3a, 0x58, 0xe4, 0x76, 0x48***REMOVED***,
	***REMOVED***,
***REMOVED***

func TestXSalsa20(t *testing.T) ***REMOVED***
	var key [32]byte

	for i, test := range xSalsa20TestData ***REMOVED***
		out := make([]byte, len(test.in))
		copy(key[:], test.key)
		XORKeyStream(out, test.in, test.nonce, &key)
		if !bytes.Equal(out, test.out) ***REMOVED***
...
func BenchmarkXOR1K(b *testing.B) ***REMOVED***
	b.StopTimer()
	out := make([]byte, 1024)
	b.StartTimer()
