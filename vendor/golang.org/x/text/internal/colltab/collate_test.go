...
func TestNonDigits(t *testing.T) ***REMOVED***
	...
	for r, hi := rune(unicode.N.R16[0].Lo), rune(unicode.N.R32[0].Hi); r <= hi; r++ ***REMOVED***
		if unicode.In(r, unicode.Nd) || !unicode.In(r, assigned) ***REMOVED***
			continue
		***REMOVED***
		if a := string(r); c.CompareString(a, "0") != -1 && c.CompareString(a, "999999") != 1 ***REMOVED***
			t.Errorf("%+q non-digit number is collated as digit", a)
		***REMOVED***
	***REMOVED***
***REMOVED***

func TestNumericCompare(t *testing.T) ***REMOVED***
	c := collate.New(language.English, collate.Loose, collate.Numeric)

	// Iterate over all digits.
	for _, r16 := range unicode.Nd.R16 ***REMOVED***
		testDigitCompare(t, c, rune(r16.Lo), rune(r16.Hi))
	***REMOVED***
	for _, r32 := range unicode.Nd.R32 ***REMOVED***
		testDigitCompare(t, c, rune(r32.Lo), rune(r32.Hi))
	***REMOVED***
***REMOVED***

func testDigitCompare(t *testing.T, c *collate.Collator, zero, nine rune) ***REMOVED***
	if !unicode.In(zero, assigned) ***REMOVED***
		return
	***REMOVED***
	n := int(nine - zero + 1)
	if n%10 != 0 ***REMOVED***
...
func BenchmarkNumericWeighter(b *testing.B) ***REMOVED***
