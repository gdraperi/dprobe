// Copyright 2014 The Go Authors. All rights reserved.
// Use of this source code is governed by a BSD-style
// license that can be found in the LICENSE file.

package webdav

import (
	"fmt"
	"math/rand"
	"path"
	"reflect"
	"sort"
	"strconv"
	"strings"
	"testing"
	"time"
)

func TestWalkToRoot(t *testing.T) ***REMOVED***
	testCases := []struct ***REMOVED***
		name string
		want []string
	***REMOVED******REMOVED******REMOVED***
		"/a/b/c/d",
		[]string***REMOVED***
			"/a/b/c/d",
			"/a/b/c",
			"/a/b",
			"/a",
			"/",
		***REMOVED***,
	***REMOVED***, ***REMOVED***
		"/a",
		[]string***REMOVED***
			"/a",
			"/",
		***REMOVED***,
	***REMOVED***, ***REMOVED***
		"/",
		[]string***REMOVED***
			"/",
		***REMOVED***,
	***REMOVED******REMOVED***

	for _, tc := range testCases ***REMOVED***
		var got []string
		if !walkToRoot(tc.name, func(name0 string, first bool) bool ***REMOVED***
			if first != (len(got) == 0) ***REMOVED***
				t.Errorf("name=%q: first=%t but len(got)==%d", tc.name, first, len(got))
				return false
			***REMOVED***
			got = append(got, name0)
			return true
		***REMOVED***) ***REMOVED***
			continue
		***REMOVED***
		if !reflect.DeepEqual(got, tc.want) ***REMOVED***
			t.Errorf("name=%q:\ngot  %q\nwant %q", tc.name, got, tc.want)
		***REMOVED***
	***REMOVED***
***REMOVED***

var lockTestDurations = []time.Duration***REMOVED***
	infiniteTimeout, // infiniteTimeout means to never expire.
	0,               // A zero duration means to expire immediately.
	100 * time.Hour, // A very large duration will not expire in these tests.
***REMOVED***

// lockTestNames are the names of a set of mutually compatible locks. For each
// name fragment:
//	- _ means no explicit lock.
//	- i means an infinite-depth lock,
//	- z means a zero-depth lock,
var lockTestNames = []string***REMOVED***
	"/_/_/_/_/z",
	"/_/_/i",
	"/_/z",
	"/_/z/i",
	"/_/z/z",
	"/_/z/_/i",
	"/_/z/_/z",
	"/i",
	"/z",
	"/z/_/i",
	"/z/_/z",
***REMOVED***

func lockTestZeroDepth(name string) bool ***REMOVED***
	switch name[len(name)-1] ***REMOVED***
	case 'i':
		return false
	case 'z':
		return true
	***REMOVED***
	panic(fmt.Sprintf("lock name %q did not end with 'i' or 'z'", name))
***REMOVED***

func TestMemLSCanCreate(t *testing.T) ***REMOVED***
	now := time.Unix(0, 0)
	m := NewMemLS().(*memLS)

	for _, name := range lockTestNames ***REMOVED***
		_, err := m.Create(now, LockDetails***REMOVED***
			Root:      name,
			Duration:  infiniteTimeout,
			ZeroDepth: lockTestZeroDepth(name),
		***REMOVED***)
		if err != nil ***REMOVED***
			t.Fatalf("creating lock for %q: %v", name, err)
		***REMOVED***
	***REMOVED***

	wantCanCreate := func(name string, zeroDepth bool) bool ***REMOVED***
		for _, n := range lockTestNames ***REMOVED***
			switch ***REMOVED***
			case n == name:
				// An existing lock has the same name as the proposed lock.
				return false
			case strings.HasPrefix(n, name):
				// An existing lock would be a child of the proposed lock,
				// which conflicts if the proposed lock has infinite depth.
				if !zeroDepth ***REMOVED***
					return false
				***REMOVED***
			case strings.HasPrefix(name, n):
				// An existing lock would be an ancestor of the proposed lock,
				// which conflicts if the ancestor has infinite depth.
				if n[len(n)-1] == 'i' ***REMOVED***
					return false
				***REMOVED***
			***REMOVED***
		***REMOVED***
		return true
	***REMOVED***

	var check func(int, string)
	check = func(recursion int, name string) ***REMOVED***
		for _, zeroDepth := range []bool***REMOVED***false, true***REMOVED*** ***REMOVED***
			got := m.canCreate(name, zeroDepth)
			want := wantCanCreate(name, zeroDepth)
			if got != want ***REMOVED***
				t.Errorf("canCreate name=%q zeroDepth=%t: got %t, want %t", name, zeroDepth, got, want)
			***REMOVED***
		***REMOVED***
		if recursion == 6 ***REMOVED***
			return
		***REMOVED***
		if name != "/" ***REMOVED***
			name += "/"
		***REMOVED***
		for _, c := range "_iz" ***REMOVED***
			check(recursion+1, name+string(c))
		***REMOVED***
	***REMOVED***
	check(0, "/")
***REMOVED***

func TestMemLSLookup(t *testing.T) ***REMOVED***
	now := time.Unix(0, 0)
	m := NewMemLS().(*memLS)

	badToken := m.nextToken()
	t.Logf("badToken=%q", badToken)

	for _, name := range lockTestNames ***REMOVED***
		token, err := m.Create(now, LockDetails***REMOVED***
			Root:      name,
			Duration:  infiniteTimeout,
			ZeroDepth: lockTestZeroDepth(name),
		***REMOVED***)
		if err != nil ***REMOVED***
			t.Fatalf("creating lock for %q: %v", name, err)
		***REMOVED***
		t.Logf("%-15q -> node=%p token=%q", name, m.byName[name], token)
	***REMOVED***

	baseNames := append([]string***REMOVED***"/a", "/b/c"***REMOVED***, lockTestNames...)
	for _, baseName := range baseNames ***REMOVED***
		for _, suffix := range []string***REMOVED***"", "/0", "/1/2/3"***REMOVED*** ***REMOVED***
			name := baseName + suffix

			goodToken := ""
			base := m.byName[baseName]
			if base != nil && (suffix == "" || !lockTestZeroDepth(baseName)) ***REMOVED***
				goodToken = base.token
			***REMOVED***

			for _, token := range []string***REMOVED***badToken, goodToken***REMOVED*** ***REMOVED***
				if token == "" ***REMOVED***
					continue
				***REMOVED***

				got := m.lookup(name, Condition***REMOVED***Token: token***REMOVED***)
				want := base
				if token == badToken ***REMOVED***
					want = nil
				***REMOVED***
				if got != want ***REMOVED***
					t.Errorf("name=%-20qtoken=%q (bad=%t): got %p, want %p",
						name, token, token == badToken, got, want)
				***REMOVED***
			***REMOVED***
		***REMOVED***
	***REMOVED***
***REMOVED***

func TestMemLSConfirm(t *testing.T) ***REMOVED***
	now := time.Unix(0, 0)
	m := NewMemLS().(*memLS)
	alice, err := m.Create(now, LockDetails***REMOVED***
		Root:      "/alice",
		Duration:  infiniteTimeout,
		ZeroDepth: false,
	***REMOVED***)
	tweedle, err := m.Create(now, LockDetails***REMOVED***
		Root:      "/tweedle",
		Duration:  infiniteTimeout,
		ZeroDepth: false,
	***REMOVED***)
	if err != nil ***REMOVED***
		t.Fatalf("Create: %v", err)
	***REMOVED***
	if err := m.consistent(); err != nil ***REMOVED***
		t.Fatalf("Create: inconsistent state: %v", err)
	***REMOVED***

	// Test a mismatch between name and condition.
	_, err = m.Confirm(now, "/tweedle/dee", "", Condition***REMOVED***Token: alice***REMOVED***)
	if err != ErrConfirmationFailed ***REMOVED***
		t.Fatalf("Confirm (mismatch): got %v, want ErrConfirmationFailed", err)
	***REMOVED***
	if err := m.consistent(); err != nil ***REMOVED***
		t.Fatalf("Confirm (mismatch): inconsistent state: %v", err)
	***REMOVED***

	// Test two names (that fall under the same lock) in the one Confirm call.
	release, err := m.Confirm(now, "/tweedle/dee", "/tweedle/dum", Condition***REMOVED***Token: tweedle***REMOVED***)
	if err != nil ***REMOVED***
		t.Fatalf("Confirm (twins): %v", err)
	***REMOVED***
	if err := m.consistent(); err != nil ***REMOVED***
		t.Fatalf("Confirm (twins): inconsistent state: %v", err)
	***REMOVED***
	release()
	if err := m.consistent(); err != nil ***REMOVED***
		t.Fatalf("release (twins): inconsistent state: %v", err)
	***REMOVED***

	// Test the same two names in overlapping Confirm / release calls.
	releaseDee, err := m.Confirm(now, "/tweedle/dee", "", Condition***REMOVED***Token: tweedle***REMOVED***)
	if err != nil ***REMOVED***
		t.Fatalf("Confirm (sequence #0): %v", err)
	***REMOVED***
	if err := m.consistent(); err != nil ***REMOVED***
		t.Fatalf("Confirm (sequence #0): inconsistent state: %v", err)
	***REMOVED***

	_, err = m.Confirm(now, "/tweedle/dum", "", Condition***REMOVED***Token: tweedle***REMOVED***)
	if err != ErrConfirmationFailed ***REMOVED***
		t.Fatalf("Confirm (sequence #1): got %v, want ErrConfirmationFailed", err)
	***REMOVED***
	if err := m.consistent(); err != nil ***REMOVED***
		t.Fatalf("Confirm (sequence #1): inconsistent state: %v", err)
	***REMOVED***

	releaseDee()
	if err := m.consistent(); err != nil ***REMOVED***
		t.Fatalf("release (sequence #2): inconsistent state: %v", err)
	***REMOVED***

	releaseDum, err := m.Confirm(now, "/tweedle/dum", "", Condition***REMOVED***Token: tweedle***REMOVED***)
	if err != nil ***REMOVED***
		t.Fatalf("Confirm (sequence #3): %v", err)
	***REMOVED***
	if err := m.consistent(); err != nil ***REMOVED***
		t.Fatalf("Confirm (sequence #3): inconsistent state: %v", err)
	***REMOVED***

	// Test that you can't unlock a held lock.
	err = m.Unlock(now, tweedle)
	if err != ErrLocked ***REMOVED***
		t.Fatalf("Unlock (sequence #4): got %v, want ErrLocked", err)
	***REMOVED***

	releaseDum()
	if err := m.consistent(); err != nil ***REMOVED***
		t.Fatalf("release (sequence #5): inconsistent state: %v", err)
	***REMOVED***

	err = m.Unlock(now, tweedle)
	if err != nil ***REMOVED***
		t.Fatalf("Unlock (sequence #6): %v", err)
	***REMOVED***
	if err := m.consistent(); err != nil ***REMOVED***
		t.Fatalf("Unlock (sequence #6): inconsistent state: %v", err)
	***REMOVED***
***REMOVED***

func TestMemLSNonCanonicalRoot(t *testing.T) ***REMOVED***
	now := time.Unix(0, 0)
	m := NewMemLS().(*memLS)
	token, err := m.Create(now, LockDetails***REMOVED***
		Root:     "/foo/./bar//",
		Duration: 1 * time.Second,
	***REMOVED***)
	if err != nil ***REMOVED***
		t.Fatalf("Create: %v", err)
	***REMOVED***
	if err := m.consistent(); err != nil ***REMOVED***
		t.Fatalf("Create: inconsistent state: %v", err)
	***REMOVED***
	if err := m.Unlock(now, token); err != nil ***REMOVED***
		t.Fatalf("Unlock: %v", err)
	***REMOVED***
	if err := m.consistent(); err != nil ***REMOVED***
		t.Fatalf("Unlock: inconsistent state: %v", err)
	***REMOVED***
***REMOVED***

func TestMemLSExpiry(t *testing.T) ***REMOVED***
	m := NewMemLS().(*memLS)
	testCases := []string***REMOVED***
		"setNow 0",
		"create /a.5",
		"want /a.5",
		"create /c.6",
		"want /a.5 /c.6",
		"create /a/b.7",
		"want /a.5 /a/b.7 /c.6",
		"setNow 4",
		"want /a.5 /a/b.7 /c.6",
		"setNow 5",
		"want /a/b.7 /c.6",
		"setNow 6",
		"want /a/b.7",
		"setNow 7",
		"want ",
		"setNow 8",
		"want ",
		"create /a.12",
		"create /b.13",
		"create /c.15",
		"create /a/d.16",
		"want /a.12 /a/d.16 /b.13 /c.15",
		"refresh /a.14",
		"want /a.14 /a/d.16 /b.13 /c.15",
		"setNow 12",
		"want /a.14 /a/d.16 /b.13 /c.15",
		"setNow 13",
		"want /a.14 /a/d.16 /c.15",
		"setNow 14",
		"want /a/d.16 /c.15",
		"refresh /a/d.20",
		"refresh /c.20",
		"want /a/d.20 /c.20",
		"setNow 20",
		"want ",
	***REMOVED***

	tokens := map[string]string***REMOVED******REMOVED***
	zTime := time.Unix(0, 0)
	now := zTime
	for i, tc := range testCases ***REMOVED***
		j := strings.IndexByte(tc, ' ')
		if j < 0 ***REMOVED***
			t.Fatalf("test case #%d %q: invalid command", i, tc)
		***REMOVED***
		op, arg := tc[:j], tc[j+1:]
		switch op ***REMOVED***
		default:
			t.Fatalf("test case #%d %q: invalid operation %q", i, tc, op)

		case "create", "refresh":
			parts := strings.Split(arg, ".")
			if len(parts) != 2 ***REMOVED***
				t.Fatalf("test case #%d %q: invalid create", i, tc)
			***REMOVED***
			root := parts[0]
			d, err := strconv.Atoi(parts[1])
			if err != nil ***REMOVED***
				t.Fatalf("test case #%d %q: invalid duration", i, tc)
			***REMOVED***
			dur := time.Unix(0, 0).Add(time.Duration(d) * time.Second).Sub(now)

			switch op ***REMOVED***
			case "create":
				token, err := m.Create(now, LockDetails***REMOVED***
					Root:      root,
					Duration:  dur,
					ZeroDepth: true,
				***REMOVED***)
				if err != nil ***REMOVED***
					t.Fatalf("test case #%d %q: Create: %v", i, tc, err)
				***REMOVED***
				tokens[root] = token

			case "refresh":
				token := tokens[root]
				if token == "" ***REMOVED***
					t.Fatalf("test case #%d %q: no token for %q", i, tc, root)
				***REMOVED***
				got, err := m.Refresh(now, token, dur)
				if err != nil ***REMOVED***
					t.Fatalf("test case #%d %q: Refresh: %v", i, tc, err)
				***REMOVED***
				want := LockDetails***REMOVED***
					Root:      root,
					Duration:  dur,
					ZeroDepth: true,
				***REMOVED***
				if got != want ***REMOVED***
					t.Fatalf("test case #%d %q:\ngot  %v\nwant %v", i, tc, got, want)
				***REMOVED***
			***REMOVED***

		case "setNow":
			d, err := strconv.Atoi(arg)
			if err != nil ***REMOVED***
				t.Fatalf("test case #%d %q: invalid duration", i, tc)
			***REMOVED***
			now = time.Unix(0, 0).Add(time.Duration(d) * time.Second)

		case "want":
			m.mu.Lock()
			m.collectExpiredNodes(now)
			got := make([]string, 0, len(m.byToken))
			for _, n := range m.byToken ***REMOVED***
				got = append(got, fmt.Sprintf("%s.%d",
					n.details.Root, n.expiry.Sub(zTime)/time.Second))
			***REMOVED***
			m.mu.Unlock()
			sort.Strings(got)
			want := []string***REMOVED******REMOVED***
			if arg != "" ***REMOVED***
				want = strings.Split(arg, " ")
			***REMOVED***
			if !reflect.DeepEqual(got, want) ***REMOVED***
				t.Fatalf("test case #%d %q:\ngot  %q\nwant %q", i, tc, got, want)
			***REMOVED***
		***REMOVED***

		if err := m.consistent(); err != nil ***REMOVED***
			t.Fatalf("test case #%d %q: inconsistent state: %v", i, tc, err)
		***REMOVED***
	***REMOVED***
***REMOVED***

func TestMemLS(t *testing.T) ***REMOVED***
	now := time.Unix(0, 0)
	m := NewMemLS().(*memLS)
	rng := rand.New(rand.NewSource(0))
	tokens := map[string]string***REMOVED******REMOVED***
	nConfirm, nCreate, nRefresh, nUnlock := 0, 0, 0, 0
	const N = 2000

	for i := 0; i < N; i++ ***REMOVED***
		name := lockTestNames[rng.Intn(len(lockTestNames))]
		duration := lockTestDurations[rng.Intn(len(lockTestDurations))]
		confirmed, unlocked := false, false

		// If the name was already locked, we randomly confirm/release, refresh
		// or unlock it. Otherwise, we create a lock.
		token := tokens[name]
		if token != "" ***REMOVED***
			switch rng.Intn(3) ***REMOVED***
			case 0:
				confirmed = true
				nConfirm++
				release, err := m.Confirm(now, name, "", Condition***REMOVED***Token: token***REMOVED***)
				if err != nil ***REMOVED***
					t.Fatalf("iteration #%d: Confirm %q: %v", i, name, err)
				***REMOVED***
				if err := m.consistent(); err != nil ***REMOVED***
					t.Fatalf("iteration #%d: inconsistent state: %v", i, err)
				***REMOVED***
				release()

			case 1:
				nRefresh++
				if _, err := m.Refresh(now, token, duration); err != nil ***REMOVED***
					t.Fatalf("iteration #%d: Refresh %q: %v", i, name, err)
				***REMOVED***

			case 2:
				unlocked = true
				nUnlock++
				if err := m.Unlock(now, token); err != nil ***REMOVED***
					t.Fatalf("iteration #%d: Unlock %q: %v", i, name, err)
				***REMOVED***
			***REMOVED***

		***REMOVED*** else ***REMOVED***
			nCreate++
			var err error
			token, err = m.Create(now, LockDetails***REMOVED***
				Root:      name,
				Duration:  duration,
				ZeroDepth: lockTestZeroDepth(name),
			***REMOVED***)
			if err != nil ***REMOVED***
				t.Fatalf("iteration #%d: Create %q: %v", i, name, err)
			***REMOVED***
		***REMOVED***

		if !confirmed ***REMOVED***
			if duration == 0 || unlocked ***REMOVED***
				// A zero-duration lock should expire immediately and is
				// effectively equivalent to being unlocked.
				tokens[name] = ""
			***REMOVED*** else ***REMOVED***
				tokens[name] = token
			***REMOVED***
		***REMOVED***

		if err := m.consistent(); err != nil ***REMOVED***
			t.Fatalf("iteration #%d: inconsistent state: %v", i, err)
		***REMOVED***
	***REMOVED***

	if nConfirm < N/10 ***REMOVED***
		t.Fatalf("too few Confirm calls: got %d, want >= %d", nConfirm, N/10)
	***REMOVED***
	if nCreate < N/10 ***REMOVED***
		t.Fatalf("too few Create calls: got %d, want >= %d", nCreate, N/10)
	***REMOVED***
	if nRefresh < N/10 ***REMOVED***
		t.Fatalf("too few Refresh calls: got %d, want >= %d", nRefresh, N/10)
	***REMOVED***
	if nUnlock < N/10 ***REMOVED***
		t.Fatalf("too few Unlock calls: got %d, want >= %d", nUnlock, N/10)
	***REMOVED***
***REMOVED***

func (m *memLS) consistent() error ***REMOVED***
	m.mu.Lock()
	defer m.mu.Unlock()

	// If m.byName is non-empty, then it must contain an entry for the root "/",
	// and its refCount should equal the number of locked nodes.
	if len(m.byName) > 0 ***REMOVED***
		n := m.byName["/"]
		if n == nil ***REMOVED***
			return fmt.Errorf(`non-empty m.byName does not contain the root "/"`)
		***REMOVED***
		if n.refCount != len(m.byToken) ***REMOVED***
			return fmt.Errorf("root node refCount=%d, differs from len(m.byToken)=%d", n.refCount, len(m.byToken))
		***REMOVED***
	***REMOVED***

	for name, n := range m.byName ***REMOVED***
		// The map keys should be consistent with the node's copy of the key.
		if n.details.Root != name ***REMOVED***
			return fmt.Errorf("node name %q != byName map key %q", n.details.Root, name)
		***REMOVED***

		// A name must be clean, and start with a "/".
		if len(name) == 0 || name[0] != '/' ***REMOVED***
			return fmt.Errorf(`node name %q does not start with "/"`, name)
		***REMOVED***
		if name != path.Clean(name) ***REMOVED***
			return fmt.Errorf(`node name %q is not clean`, name)
		***REMOVED***

		// A node's refCount should be positive.
		if n.refCount <= 0 ***REMOVED***
			return fmt.Errorf("non-positive refCount for node at name %q", name)
		***REMOVED***

		// A node's refCount should be the number of self-or-descendents that
		// are locked (i.e. have a non-empty token).
		var list []string
		for name0, n0 := range m.byName ***REMOVED***
			// All of lockTestNames' name fragments are one byte long: '_', 'i' or 'z',
			// so strings.HasPrefix is equivalent to self-or-descendent name match.
			// We don't have to worry about "/foo/bar" being a false positive match
			// for "/foo/b".
			if strings.HasPrefix(name0, name) && n0.token != "" ***REMOVED***
				list = append(list, name0)
			***REMOVED***
		***REMOVED***
		if n.refCount != len(list) ***REMOVED***
			sort.Strings(list)
			return fmt.Errorf("node at name %q has refCount %d but locked self-or-descendents are %q (len=%d)",
				name, n.refCount, list, len(list))
		***REMOVED***

		// A node n is in m.byToken if it has a non-empty token.
		if n.token != "" ***REMOVED***
			if _, ok := m.byToken[n.token]; !ok ***REMOVED***
				return fmt.Errorf("node at name %q has token %q but not in m.byToken", name, n.token)
			***REMOVED***
		***REMOVED***

		// A node n is in m.byExpiry if it has a non-negative byExpiryIndex.
		if n.byExpiryIndex >= 0 ***REMOVED***
			if n.byExpiryIndex >= len(m.byExpiry) ***REMOVED***
				return fmt.Errorf("node at name %q has byExpiryIndex %d but m.byExpiry has length %d", name, n.byExpiryIndex, len(m.byExpiry))
			***REMOVED***
			if n != m.byExpiry[n.byExpiryIndex] ***REMOVED***
				return fmt.Errorf("node at name %q has byExpiryIndex %d but that indexes a different node", name, n.byExpiryIndex)
			***REMOVED***
		***REMOVED***
	***REMOVED***

	for token, n := range m.byToken ***REMOVED***
		// The map keys should be consistent with the node's copy of the key.
		if n.token != token ***REMOVED***
			return fmt.Errorf("node token %q != byToken map key %q", n.token, token)
		***REMOVED***

		// Every node in m.byToken is in m.byName.
		if _, ok := m.byName[n.details.Root]; !ok ***REMOVED***
			return fmt.Errorf("node at name %q in m.byToken but not in m.byName", n.details.Root)
		***REMOVED***
	***REMOVED***

	for i, n := range m.byExpiry ***REMOVED***
		// The slice indices should be consistent with the node's copy of the index.
		if n.byExpiryIndex != i ***REMOVED***
			return fmt.Errorf("node byExpiryIndex %d != byExpiry slice index %d", n.byExpiryIndex, i)
		***REMOVED***

		// Every node in m.byExpiry is in m.byName.
		if _, ok := m.byName[n.details.Root]; !ok ***REMOVED***
			return fmt.Errorf("node at name %q in m.byExpiry but not in m.byName", n.details.Root)
		***REMOVED***

		// No node in m.byExpiry should be held.
		if n.held ***REMOVED***
			return fmt.Errorf("node at name %q in m.byExpiry is held", n.details.Root)
		***REMOVED***
	***REMOVED***
	return nil
***REMOVED***

func TestParseTimeout(t *testing.T) ***REMOVED***
	testCases := []struct ***REMOVED***
		s       string
		want    time.Duration
		wantErr error
	***REMOVED******REMOVED******REMOVED***
		"",
		infiniteTimeout,
		nil,
	***REMOVED***, ***REMOVED***
		"Infinite",
		infiniteTimeout,
		nil,
	***REMOVED***, ***REMOVED***
		"Infinitesimal",
		0,
		errInvalidTimeout,
	***REMOVED***, ***REMOVED***
		"infinite",
		0,
		errInvalidTimeout,
	***REMOVED***, ***REMOVED***
		"Second-0",
		0 * time.Second,
		nil,
	***REMOVED***, ***REMOVED***
		"Second-123",
		123 * time.Second,
		nil,
	***REMOVED***, ***REMOVED***
		"  Second-456    ",
		456 * time.Second,
		nil,
	***REMOVED***, ***REMOVED***
		"Second-4100000000",
		4100000000 * time.Second,
		nil,
	***REMOVED***, ***REMOVED***
		"junk",
		0,
		errInvalidTimeout,
	***REMOVED***, ***REMOVED***
		"Second-",
		0,
		errInvalidTimeout,
	***REMOVED***, ***REMOVED***
		"Second--1",
		0,
		errInvalidTimeout,
	***REMOVED***, ***REMOVED***
		"Second--123",
		0,
		errInvalidTimeout,
	***REMOVED***, ***REMOVED***
		"Second-+123",
		0,
		errInvalidTimeout,
	***REMOVED***, ***REMOVED***
		"Second-0x123",
		0,
		errInvalidTimeout,
	***REMOVED***, ***REMOVED***
		"second-123",
		0,
		errInvalidTimeout,
	***REMOVED***, ***REMOVED***
		"Second-4294967295",
		4294967295 * time.Second,
		nil,
	***REMOVED***, ***REMOVED***
		// Section 10.7 says that "The timeout value for TimeType "Second"
		// must not be greater than 2^32-1."
		"Second-4294967296",
		0,
		errInvalidTimeout,
	***REMOVED***, ***REMOVED***
		// This test case comes from section 9.10.9 of the spec. It says,
		//
		// "In this request, the client has specified that it desires an
		// infinite-length lock, if available, otherwise a timeout of 4.1
		// billion seconds, if available."
		//
		// The Go WebDAV package always supports infinite length locks,
		// and ignores the fallback after the comma.
		"Infinite, Second-4100000000",
		infiniteTimeout,
		nil,
	***REMOVED******REMOVED***

	for _, tc := range testCases ***REMOVED***
		got, gotErr := parseTimeout(tc.s)
		if got != tc.want || gotErr != tc.wantErr ***REMOVED***
			t.Errorf("parsing %q:\ngot  %v, %v\nwant %v, %v", tc.s, got, gotErr, tc.want, tc.wantErr)
		***REMOVED***
	***REMOVED***
***REMOVED***
